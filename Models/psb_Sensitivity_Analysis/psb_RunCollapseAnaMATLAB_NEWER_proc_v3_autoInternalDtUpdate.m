...
%
% -------------------
function psb_RunCollapseAnaMATLAB_NEWER_proc_v3_autoInternalDtUpdate(dtForCollapseMATLAB, minStoryDriftRatioForCollapseMATLAB, elementUsedForColSensModelMATLAB, ...
    ...
    maxNumRuns, pertubationForNonConvSingular, flagForEQFileFormat, periodUsedForScalingGroundMotions, dampingRatioUsedForSaDef, ...
    extraSecondsToRunAnalysis, timeTakenInMinsForEachAnalysis, eqTimeHistoryPreFormatted, openseesFileToUse, dtForConvergenceMATLAB)
...
%	(4-30-16, PSB) implemented to debug for the cases when curtailed ground motions are used and some goof up occurs.    
            numPointsFromLengthOfTH = size(load(fullfile('C:\OpenSeesProcessingFiles\EQs', sprintf('SortedEQFile_(%d).txt', eqNumber))), 1);
            numPointsFromFile = load(fullfile('C:\OpenSeesProcessingFiles\EQs', sprintf('NumPointsFile_(%d).txt', eqNumber)));
            ...
            if (numPointsFromLengthOfTH ~= numPointsFromFile) 
                ...
                dtOfTimeHistory(indexForEQ) = load(fullfile('C:\OpenSeesProcessingFiles\EQs', sprintf('DtFile_(%d).txt',eqNumber)));
...
%         fprintf([dtOfTimeHistory, matrixOfDtForCollapseMATLAB])

% (11-23-15, PSB) removed temporarily. This implies that recover thing
% won't worl, however will store the array of timeTakenInEachAnalyses
                numberOfAnalysesRun = length(timeTakenInMinsForEachAnalysis);
%     timeTakenInMinsForEachAnalysis = zeros(length(eqNumberLIST),1);
    sensDir = pwd;           
                
    % (11-21-15, PSB) copying the file for independen earthquake analyses corresponding to different GMs.
...
%         cd .. % back to the Prak_Sensitivity_Analysis folder
	...
	% (11-21-15, PSB) creating model specific files for parallel computing. This is to keep all the earthqauake analyses independent of one another. 
    ...
    for eqNumberIndex = 1:length(eqNumberLIST)
        ...
        copyfile('*.tcl', addressForModelDir); 
                ...
                % if some other extension files are to be copied as well, it is receommended to give the command again, for ex- copyfile('*.m','.') etc 
...
%     dtForCollapseMATLAB = matrixOfDtForCollapseMATLAB(indexForTheRunningEQ, 2);
    ...
    fprintf(myFileStream, '# dt ground motion is %f sec.\n', dtOfTimeHistory(eqNumberIndex));
    fprintf(myFileStream, 'set dtForConvergenceLIST {');
    for i = 1:length(dtForConvergenceMATLAB)
        fprintf(myFileStream, '%f ', dtOfTimeHistory(eqNumberIndex)/dtForConvergenceMATLAB(i));
    end
    fprintf(myFileStream, '} \n');
...
%%
                            ...
                            if (isCollapsed == -1 || isSingular == -1 || isNonConv == -1 || maxStoryDriftRatioForFullStr == -1 || minStoryDriftRatioForFullStr == -1)
                                error('ERROR: The variables are still equal to their initialized values, even after running the EQ (first loop)!!!');
                            end
                        ...
                        % Update the variable values for the collapse range, if the analysis is not singular and if it was fully converged
                            if (isSingular == 0 && isNonConv == 0)
                                ...
                                if (isCollapsed == 1)
                                    % It collapsed and converged, so alter the upper bound on the collapse region  
                                    disp('It collapsed!');   
                                    fprintf('Reached here. Bookmark_prak- 1 \n');
            ...
            % The following else condition i.e. when either isSingular or isNonConv is 1 is expanded below with reduction in $dt_ana
...
%                                     currentSaLevel = maxSaNoCollapse + pertubationForNonConvSingular; % Changed to increase Ss due to convergence issued on 9-19-05
                            end
...
%                             end
                            ...
                            % If it's singular or non-converged, change the dt_analysis time step and rerun, this will take care of nonconvergence cases 
                            if (isSingular == 1 || isNonConv == 1)
%%%%%%%%%%%%%%%%%%%%%% START OF WORKING ON AUTO UPDATE OF DT %%%%%%%%%%%%%%%%%%%%%% 
            % (4-5-17, PSB) some model with Elwood spring show non-convergence for much smaller ground motion (as low as for the very intial Sa Value) 
            % Therefore, for every non-convergence case, let's update the dt_analysis and run with the revised dt value
            % these dt_analysis values shall be set back after the solution converges.
            
                                if (isCollapsed == 1)
                                    disp('It collapsed!');
                                    fprintf('Reached here. Bookmark_prak- 2 \n');
                                    cd .. % note that cd .. in the end of this while loop doesnt get executed if collapse is found, so we need to give this command here for the collapse case
                                    minSaCollapse = currentSaLevel;
                                    saLevelIndex = saLevelIndex + 1;
                                    break;
                                end
%% No using the following reduction in time step of analysis because 
% now I have coded it within opensees to selectively reduce analysis time
% step during the time interval where non-convergence is occurring.
            
%                                 if (dtForCollapseMATLAB >= 1)
%                                     if (dtForCollapseLIST(eqNumberIndex) - dtOfTimeHistory(eqNumberIndex) / 2) > -1e-7 % equivalent code for being slightly more than or equal to the second variable
%                                         dtForCollapseLIST(eqNumberIndex) = dtOfTimeHistory(eqNumberIndex) / 5;
%                                     elseif (dtForCollapseLIST(eqNumberIndex) - dtOfTimeHistory(eqNumberIndex) / 5) > -1e-7
%                                         dtForCollapseLIST(eqNumberIndex) = dtOfTimeHistory(eqNumberIndex) / 10;
% %                                     elseif (dtForCollapseLIST(eqNumberIndex) - dtOfTimeHistory(eqNumberIndex) / 10) > -1e-7
% %                                         dtForCollapseLIST(eqNumberIndex) = dtOfTimeHistory(eqNumberIndex) / 20;
% %                                     elseif (dtForCollapseLIST(eqNumberIndex) - dtOfTimeHistory(eqNumberIndex) / 20) > -1e-7
% %                                         dtForCollapseLIST(eqNumberIndex) = dtOfTimeHistory(eqNumberIndex) / 30; # keeping all intermediate increments will add significantly to the run time for actual collapse cases;
% %                                     elseif (dtForCollapseLIST(eqNumberIndex) - dtOfTimeHistory(eqNumberIndex) / 30) > -1e-7
% %                                         dtForCollapseLIST(eqNumberIndex) = dtOfTimeHistory(eqNumberIndex) / 40;
% %                                     elseif (dtForCollapseLIST(eqNumberIndex) - dtOfTimeHistory(eqNumberIndex) / 40) > -1e-7
% %                                         dtForCollapseLIST(eqNumberIndex) = dtOfTimeHistory(eqNumberIndex) / 50;
%                                     else
%                                 % If it is still singular or non-converged DESPITE REDUCING THE ANALYSIS TIME STEPS TO A VERY SMALL VALUE
%                                 % then reduce Sa by pertubationForNonConvSingular and rerun.
% %                                         if (isSingular == 1 || isNonConv == 1)
%                                             % It is either singular or non-converged, so reduce Sa a litte and run it again
%                                             
% %                                                 currentSaLevel = currentSaLevel + pertubationForNonConvSingular; % Changed to increase Ss due to convergence issued on 9-19-05
% %                                                 saLevelIndex = saLevelIndex + 1;                 % Increment the Sa level index
% 
% %                                         end
%                             
%                             % It collapsed; update the variables, then break from the while loop
%                                         if (isCollapsed == 1) 
%                                             disp('It collapsed!');
%                                             fprintf('Reached here. Bookmark_prak- 2 \n'); 
%                                             cd .. % note that cd .. in the end of this while loop doesnt get executed if collapse is found, so we need to give this command here for the collapse case
...
%                                     
%                             fprintf('!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!! \n');
%                             fprintf('     Changed dt_for_analysis value to %f sec \n', dtForCollapseLIST(eqNumberIndex));
%                             fprintf('!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!! \n');
%                             
%                             startDir = pwd;
%                             fprintf('current directory is %s', startDir); 
%                             cd ../../../Output/
%                             mkdir('ChangedDtAnalysisDuringAnalysis')
%                             cd ChangedDtAnalysisDuringAnalysis/
% 
%                             dtFileName = sprintf('dtAnalysisRevised_%s.txt', sensModel);
%                             fid = fopen(dtFileName, 'a'); % opening the file in append mode 
%                             fprintf(fid, '####################################################################### \n');
%                             fprintf(fid, 'For eqNumber- %i and SaValue of- %.2fg:  \n', eqNumber, currentSaLevel);
%                             fprintf(fid, 'Time step of ground motion is %f sec. \n', dtOfTimeHistory(eqNumberIndex));
%                             fprintf(fid, 'Initial dt analysis is %f sec. \n', dtOfTimeHistory(eqNumberIndex)/dtForCollapseMATLAB);
%                             fprintf(fid, 'And updated dt analysis is %f sec. \n', dtForCollapseLIST(eqNumberIndex));
% %                             fprintf(fid, '####################################################################### \n');
%                             fclose(fid);
% 		
%                             cd(startDir)
%                             
%                                   
%                                 % dtForCollapseMATLAB1 = dtForCollapseLIST(eqNumberIndex); % this is being assigned while writing to the VarDefinitionsFile.tcl 
%                                 
%                                 currentSaLevel = currentSaLevel + 0; 
%                                 saLevelIndex = saLevelIndex + 0; % i.e. no change in the currentSaLevel, we run the same analysis with a different analysis time step
%                                 
% %                                 dtForCollapseLIST(eqNumberIndex) = dtOfTimeHistory(eqNumberIndex)/dtForCollapseMATLAB;
%                                 end
                            else
                                    % if not singular or non-convergent then reset the dtForCollapseLIST parameter (in case it has been altered due to the above code)
                                dtForCollapseLIST(eqNumberIndex) = dtOfTimeHistory(eqNumberIndex)/dtForCollapseMATLAB;
                                    % Not singular, so increment Sa level
                                currentSaLevel = currentSaLevel + startStepSize;
                                    % textToDisp=sprintf('currentSaLevel changed to %3.2f',currentSaLevel); disp(textToDisp);
                                saLevelIndex = saLevelIndex + 1;                 % Increment the Sa level index
                            end
...
%%%%%%%%%%%%%%%%%%%%%% END OF WORKING ON AUTO UPDATE OF DT %%%%%%%%%%%%%%%%%%%%%%                                 
                            ...
                            % Note that I effectively round to two decimal places when doing this (this is the compilcated "round" stuff)
...
%                         currentSaLevel = maxSaNoCollapse + tolerance;   % Sa level index was incremeted before leaving the above loop and is also incremeted below in this loop
        ...
        if (dtForCollapseMATLAB >= 1)
                    % reset dt_analysis to what we started with, because this may have been altered in the course of last analysis.    
            dtForCollapseLIST(eqNumberIndex) = dtOfTimeHistory(eqNumberIndex)/dtForCollapseMATLAB;
        end
...
%                 fprintf('!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!\n');
%                 fprintf('!!!!!!!!!!!! Sa Level = %.2fg !!!!!!!!!!!!!!\n', min(abs(saLevelForEachRun - currentSaLevel)));
%                 disp(dtForCollapseLIST(eqNumberIndex));
%                 disp(dtOfTimeHistory(eqNumberIndex)/20);
%                 disp(abs(dtForCollapseLIST(eqNumberIndex) - dtOfTimeHistory(eqNumberIndex)/20) < 1e-6);
%                 fprintf('!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!\n');


                if min(abs(saLevelForEachRun - currentSaLevel)) < 1e-4 && ...
                        abs(dtForCollapseLIST(eqNumberIndex) - dtOfTimeHistory(eqNumberIndex)/20) < 1e-6
                    % (4-14-17, PSB) changing this condition to be active when Sa
                    % level is repeated and analysis time step is changed to be at the minimum value (say, dt/20)
...
% (4-22-16, PSB) This try and catch is not really reuired anymore because try command itself shall suffice. However, keeping it for now.
    fprintf('Before try and catch directory is %s \n',pwd);
    try 
        cd(specificSensFolderForSeparateEqs)
    catch 
        cd ..
    ...
    fprintf('After try and catch directory is %s \n',pwd);
...
% Reset the dtForCollapseLIST parameter (in case it has been altered due to the above code)
%     dtForCollapseLIST(eqNumberIndex) = dtOfTimeHistory(eqNumberIndex)/dtForCollapseMATLAB; % resetting here stops from reducing the dt_analysis more than once below. therefore, now reducing the dt_ana in the end of the loop
    dtForCollapseMATLAB1 = dtForCollapseLIST(eqNumberIndex);
...
%%
                        % Save the results of the EQ, in the vactors that will later be saved
                            saLevelForEachRun(saLevelIndex)     = currentSaLevel;
                            scaleFactorForEachRun(saLevelIndex) = scaleFactorForRunFromMatlab;
                            isCollapsedForEachRun(saLevelIndex) = isCollapsed;
                            isSingularForEachRun(saLevelIndex)  = isSingular;
...
%                                 dtForCollapseLIST(eqNumberIndex) = dtOfTimeHistory(eqNumberIndex)/dtForCollapseMATLAB;
                                ...
                                saLevelIndex = saLevelIndex + 1;                 % Increment the Sa level index
                        % Not singular, so increment Sa level
                            % (11-18-15, PSB) to eliminate error due to file name without 2 sig fig after decimal. Following 'if...' conditions satisfies the purpose of renaming SaFolder 
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%                                
%                                 currentSaLevel = maxSaNoCollapse + pertubationForNonConvSingular;
%                                     if min(abs(saLevelForEachRun - currentSaLevel)) < 1e-4
%                                         saLevelForEachRun
%                                         fprintf('!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!\n');
%                                         fprintf('!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!\n');
