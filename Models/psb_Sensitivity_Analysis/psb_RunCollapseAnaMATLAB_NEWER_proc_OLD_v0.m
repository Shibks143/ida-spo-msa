...
function psb_RunCollapseAnaMATLAB_NEWER_proc(dtForCollapseMATLAB, minStoryDriftRatioForCollapseMATLAB, elementUsedForColSensModelMATLAB, ...
    ...
    maxNumRuns, pertubationForNonConvSingular, flagForEQFileFormat, periodUsedForScalingGroundMotions, dampingRatioUsedForSaDef, extraSecondsToRunAnalysis, timeTakenInMinsForEachAnalysis, eqTimeHistoryPreFormatted)
...
%                 sensDirLocalForParfor = pwd;
        ...
        if(mod(currentSaLevel * 100, 10) <= 1e-4)    % this is theoretically a check for equality with zero, but due to precision error, it wont go through hence a small value
            ...
            fprintf('###########################################################################################\n');
            fprintf('############################# Cuurent Sa Level is %.4f ############################\n', currentSaLevel);
            fprintf('###########################################################################################\n');
...
%%
                            ...
                            % If it's singular or non-converged, then reduce Sa by pertubationForNonConvSingular and rerun.
...
%                             end
                        ...
                        % Break if we have run to many EQ's
                            if (saLevelIndex >= maxNumRuns) 
                                disp(['saLevelIndex = ' num2str(saLevelIndex) '. We ran the limit of EQs, so stop the analysis!!!'])
...
%                                 end
    ...
    disp(currentSaLevel)
%         if(mod(currentSaLevel * 100, 10) <= 1e-4)    % this is theoretically a check for equality with zero, but due to precision error, it wont go through hence a small value
        if(mod(round(currentSaLevel * 100), 10) < 0.01)    % this is theoretically a check for equality with zero, but due to precision error, it wont go through hence a small value 
