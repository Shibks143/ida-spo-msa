...
# Modified by: Prakash Singh of IIT Bombay
# Date: 4-30-16
# Later modified by: Shivakumar K S of IIT Madras, updated on 28-Feb-2026
#----------------------------------------------------------------------------------#
#####################################################################################################
#####################################################################################################
####################################################################################################
...
# First define the variables that will be passed back to the calling function using upvar...
	...
	upvar maxStoryResidualDriftRatioForFullStr maxStoryResidualDriftRatioForFullStrR;
	upvar minStoryResidualDriftRatioForFullStr minStoryResidualDriftRatioForFullStrR;
    upvar absStoryResidualDriftRatioForFullStr absStoryResidualDriftRatioForFullStrR;
	
	# Drift ratios at each story for this run
	upvar maxStoryDriftRatioAtEachStoryAtEachRun maxStoryDriftRatioAtEachStoryAtEachRunR
	upvar minStoryDriftRatioAtEachStoryAtEachRun minStoryDriftRatioAtEachStoryAtEachRunR
	upvar absStoryDriftRatioAtEachStoryAtEachRun absStoryDriftRatioAtEachStoryAtEachRunR

	# Residual drift ratios at each story for this run
	upvar maxStoryResidualDriftRatioAtEachStoryAtEachRun maxStoryResidualDriftRatioAtEachStoryAtEachRunR
	upvar minStoryResidualDriftRatioAtEachStoryAtEachRun minStoryResidualDriftRatioAtEachStoryAtEachRunR
	upvar absStoryResidualDriftRatioAtEachStoryAtEachRun absStoryResidualDriftRatioAtEachStoryAtEachRunR

	# Peak floor accelerations at each story for this run
	upvar maxPeakFloorAccelerationAtEachFloorAtEachRun maxPeakFloorAccelerationAtEachFloorAtEachRunR
	upvar minPeakFloorAccelerationAtEachFloorAtEachRun minPeakFloorAccelerationAtEachFloorAtEachRunR
	upvar absPeakFloorAccelerationAtEachFloorAtEachRun absPeakFloorAccelerationAtEachFloorAtEachRunR
	
	# Make eqDataFolder (set by psb_VarDefinitionsFromMATLAB.tcl at global scope) visible inside this proc
	global eqDataFolder
	
	
	# ----------------------------------------------------------
	# Clear arrays from previous run (IMPORTANT)
	# ----------------------------------------------------------
	array unset maxStoryDriftRatioAtEachStoryAtEachRunR
	array unset minStoryDriftRatioAtEachStoryAtEachRunR
	array unset absStoryDriftRatioAtEachStoryAtEachRunR

	array unset maxStoryResidualDriftRatioAtEachStoryAtEachRunR
	array unset minStoryResidualDriftRatioAtEachStoryAtEachRunR
	array unset absStoryResidualDriftRatioAtEachStoryAtEachRunR

	array unset maxPeakFloorAccelerationAtEachFloorAtEachRunR
	array unset minPeakFloorAccelerationAtEachFloorAtEachRunR
	array unset absPeakFloorAccelerationAtEachFloorAtEachRunR
...
source psb_DefineEarthquakeRecordInformation.tcl
...
# Initialize variables used for the analysis
    ...
    set maxStoryResidualDriftRatioForFullStrR   0;          # Initialize residual interstory drift ratio (post-earthquake state, evaluated after extra analysis duration); added 28-Feb-2026
    set minStoryResidualDriftRatioForFullStrR   0;          # Initialize residual interstory drift ratio (post-earthquake state, evaluated after extra analysis duration); added 28-Feb-2026
	set absStoryResidualDriftRatioForFullStrR   0;          # Initialize residual interstory drift ratio (post-earthquake state, evaluated after extra analysis duration); added 28-Feb-2026
	
	# ----------------------------------------------------------
	# Initialize per-story arrays to 0.0
	# ----------------------------------------------------------

	set numFloors  [llength $nodeNumsAtEachFloorLIST]
	set numStories [expr $numFloors - 1]

	for {set story 1} {$story <= $numStories} {incr story} {

		# Peak drift ratios
		set maxStoryDriftRatioAtEachStoryAtEachRunR($story) 0.0
		set minStoryDriftRatioAtEachStoryAtEachRunR($story) 0.0
		set absStoryDriftRatioAtEachStoryAtEachRunR($story) 0.0

		# Residual drift ratios (post-earthquake)
		set maxStoryResidualDriftRatioAtEachStoryAtEachRunR($story) 0.0
		set minStoryResidualDriftRatioAtEachStoryAtEachRunR($story) 0.0
		set absStoryResidualDriftRatioAtEachStoryAtEachRunR($story) 0.0
    }
	for {set floor 1} {$floor <= $numFloors} {incr floor} {

		set maxPeakFloorAccelerationAtEachFloorAtEachRunR($floor) 0.0
		set minPeakFloorAccelerationAtEachFloorAtEachRunR($floor) 0.0
		set absPeakFloorAccelerationAtEachFloorAtEachRunR($floor) 0.0
    }

	# (4-30-16, PSB) added this additional parameter to be able to use revised ground motion time histories when curtailment is done manually
	set seriesTag 9101
if {$eqTimeHistoryPreFormatted == 1} {
	
	# set dtFile [open " E:/StaticDynamicAnalysis/ida-spo-msa/OpenSeesProcessingFiles/EQs/DtFile_($eqNumber).txt" r]
		set dtFile [open "$eqDataFolder/DtFile_($eqNumber).txt" r]
	set dt [string trim [read $dtFile]]
	close $dtFile

	# set numPtFile [open " E:/StaticDynamicAnalysis/ida-spo-msa/OpenSeesProcessingFiles/EQs/NumPointsFile_($eqNumber).txt" r]
	set numPtFile [open "$eqDataFolder/NumPointsFile_($eqNumber).txt" r]
	set numPoints [string trim [read $numPtFile]]
	close $numPtFile

	# set Series "Path -filePath  E:/StaticDynamicAnalysis/ida-spo-msa/OpenSeesProcessingFiles/EQs/SortedEQFile_($eqNumber).txt -dt $dt -factor [expr $scaleFactorForRunFromMatlab * $g]"
	timeSeries Path $seriesTag -filePath $eqDataFolder/SortedEQFile_($eqNumber).txt -dt $dt -factor [expr $scaleFactorForRunFromMatlab * $g]
	...
	# set numPtFile [open " E:/StaticDynamicAnalysis/ida-spo-msa/OpenSeesProcessingFiles/EQs/NumPointsFile_($eqNumber).txt" r]
	# set numPtFile [open "$eqDataFolder/NumPointsFile_($eqNumber).txt" r]
	# set numPoints [read $numPtFile]
	# close $numPtFile

	# set Series "Path -filePath  E:/StaticDynamicAnalysis/ida-spo-msa/OpenSeesProcessingFiles/EQs/SortedEQFile_($eqNumber).txt -dt $dt -factor [expr $scaleFactorForRunFromMatlab * $g]"
	# set Series "Path -filePath $eqDataFolder/SortedEQFile_($eqNumber).txt -dt $dt -factor [expr $scaleFactorForRunFromMatlab * $g]"
...
# Depending on what type of record format is being used, then do different things to set up the record to run...
	if {$eqFormatForCollapseList == "Formatted_singleComp" || $eqFormatForCollapseList == "Formatted_geoMean" || $eqFormatForCollapseList == "Formatted_codeScaling"} {
		...
		set Series "Path -filePath  E:/StaticDynamicAnalysis/ida-spo-msa/OpenSeesProcessingFiles/EQs/$eqFileName($eqNumber) -dt $dtForEQ($eqNumber) -factor [expr $scaleFactorForRunFromMatlab * $g]"
		...
		# Use the procedure to get dt and numPoints for the EQ and make a file for the EQ that has a single column of accel TH in it.
			...
			# set eqFileNameForProc " E:/StaticDynamicAnalysis/ida-spo-msa/OpenSeesProcessingFiles/EQs/$eqFileName($eqNumber)"
			set eqFileNameForProc "$eqDataFolder/$eqFileName($eqNumber)"
			# set outputFileNameForProc " E:/StaticDynamicAnalysis/ida-spo-msa/OpenSeesProcessingFiles/EQs/SortedEQFile_($eqNumber).txt"
			set outputFileNameForProc "$eqDataFolder/SortedEQFile_($eqNumber).txt"
...
# Set the tag number for the EQ pattern
	set tag 101

# Define the loading patern for the EQ
	#                           tag  dir  accel    series
	pattern UniformExcitation  $tag   1  -accel   $seriesTag
...
# Initial variables before starting dynamic analysis
	...
	set maxTime [expr $currentTime + [expr ($numPoints)*$dt] + $extraSecondsToRunAnalysis];  
	set numSteps [expr 1 * [expr ($numPoints)*$dt + $extraSecondsToRunAnalysis] / [expr ($dtForAnalysis)]]
...
#	used in the analsyis will be output in the output files.
	set testTolerance [expr 1.0e-6]
	set testMinTolerance1 [expr 1.0e-5];	
	set testMinTolerance2 [expr 1.0e-4];	
	set testMinTolerance3 [expr 1.0e-3];	
	set testMinTolerance4 [expr 1.0e-2];	
	set testMinTolerance5 [expr 1.0e-1];	
...
# iterAlgo and iterAlgoArg are defined in psb_SetAnalysisOptions.tcl
...
#    		set ok [analyze 1 $dtForAnalysis]
		...
		source psb_SolutionAlgorithmSubFile.tcl
	...
	# Now for the step check for collapse or singularity; then stop anaysis is appropriate.
		# 	It is just to try to catch a singularity error before it 
		...
		#	sometimes the recorders without the singularity information have huge displacement values in them.
			...
			# If any of the displ are greater than this (abs value), then call it singular.
...
#				puts "numFloors is $numFloors"
				while {($currentFloorNum <= $numFloors)} {
					...
					# Check if the displacement is greater than any previous and alter the max. if needed.
...
#					puts "Top floor displ: $floorDisplVECTOR($floorNumTopFloorOfStory)"
				...
				# Store per-story values for this run
				set maxStoryDriftRatioAtEachStoryAtEachRunR($currentStoryNum) $storyDriftRatioVECTOR($currentStoryNum)
				set minStoryDriftRatioAtEachStoryAtEachRunR($currentStoryNum) $storyDriftRatioVECTOR($currentStoryNum)
				set absStoryDriftRatioAtEachStoryAtEachRunR($currentStoryNum) [expr abs($storyDriftRatioVECTOR($currentStoryNum))]
			...
			# Compute peak floor accelerations for this run
			set numFloors [llength $nodeNumsAtEachFloorLIST]

			for {set floor 1} {$floor <= $numFloors} {incr floor} {

				set nodeNum [lindex $nodeNumsAtEachFloorLIST [expr {$floor-1}]]

				if {$nodeNum == -1} {
					set acc 0.0
				} else {
					set acc [nodeAccel $nodeNum 1]   ;# <-- DOF 1
				}

				# Update peaks
				if {$acc > $maxPeakFloorAccelerationAtEachFloorAtEachRunR($floor)} {
					set maxPeakFloorAccelerationAtEachFloorAtEachRunR($floor) $acc
				}

				if {$acc < $minPeakFloorAccelerationAtEachFloorAtEachRunR($floor)} {
					set minPeakFloorAccelerationAtEachFloorAtEachRunR($floor) $acc
				}

				if {[expr {abs($acc)}] > $absPeakFloorAccelerationAtEachFloorAtEachRunR($floor)} {
					set absPeakFloorAccelerationAtEachFloorAtEachRunR($floor) [expr {abs($acc)}]
				}
			}
...
#---------------------------------------------------------------------------------------------------
# Compute residual interstory drift ratio (post-EQ state); included by Shivakumar K S on 28-Feb-2026
#---------------------------------------------------------------------------------------------------
set numFloors  [llength $nodeNumsAtEachFloorLIST]
set numStories [expr $numFloors - 1]

# Initialize full-structure residual drift variables
set maxStoryResidualDriftRatioForFullStrR 0.0
set minStoryResidualDriftRatioForFullStrR 0.0
set absStoryResidualDriftRatioForFullStrR 0.0

# Loop over stories to compute residuals and update full-structure stats
for {set story 1} {$story <= $numStories} {incr story} {
    set top    [expr $story + 1]
    set bottom $story

    # Compute residual drift for this story
    set residualDrift [expr \
        ($floorDisplVECTOR($top) - $floorDisplVECTOR($bottom)) / \
        ([lindex $floorHeightsLIST $top] - [lindex $floorHeightsLIST $bottom])]

    # Update full-structure max/min/abs
    if {$residualDrift > $maxStoryResidualDriftRatioForFullStrR} {
        set maxStoryResidualDriftRatioForFullStrR $residualDrift
    }
    if {$residualDrift < $minStoryResidualDriftRatioForFullStrR} {
        set minStoryResidualDriftRatioForFullStrR $residualDrift
    }
    if {[expr abs($residualDrift)] > $absStoryResidualDriftRatioForFullStrR} {
        set absStoryResidualDriftRatioForFullStrR [expr abs($residualDrift)]
    }

}

# Remove the load pattern after residual calculation
remove loadPattern $tag
loadConst -time 0.0
...
# Depending on what type of record format is being used, then do different things to set up the record to run...
	if {$eqFormatForCollapseList == "Formatted_singleComp" } {
		...
		# We need to read the dt and numPoints files that have been pre-saved in the EQ folder.
			...
			set fileID [open  E:/StaticDynamicAnalysis/ida-spo-msa/OpenSeesProcessingFiles/EQs/DtFile_($eqNumber).txt r]
			gets $fileID dt
			puts "dt is $dt"
			close $fileID

			# Load numPoints from the file (note: I do not think this is actually used in the analysis procedure, but I am not positive)
			set fileID [open  E:/StaticDynamicAnalysis/ida-spo-msa/OpenSeesProcessingFiles/EQs/NumPointsFile_($eqNumber).txt r]
		...
		# Use the procedure to get dt and numPoints for the EQ and make a file for the EQ that has a single column of accel TH in it.
			...
			if { $eqFormatForCollapseList == "PEER-NGA_singleComp" } {
				...
				#	no longer used.
				set eqFileNameForProc " E:/StaticDynamicAnalysis/ida-spo-msa/OpenSeesProcessingFiles/EQs/A_PEERNGADatabase/$eqFileName"
				set outputFileNameForProc " E:/StaticDynamicAnalysis/ida-spo-msa/OpenSeesProcessingFiles/EQs/SortedEQFile_($eqNumber).txt"
				...
				#	to a TCL script, read, then is passed this procedure .  The EQ Information file is 
				#	no longer used.
				set eqFileNameForProc " E:/StaticDynamicAnalysis/ida-spo-msa/OpenSeesProcessingFiles/EQs/A_PEERNGADatabase_Rotated/$eqFileName"
				set outputFileNameForProc " E:/StaticDynamicAnalysis/ida-spo-msa/OpenSeesProcessingFiles/EQs/SortedEQFile_($eqNumber).txt"
