...
% beam hinges
function defineBeamHingeRecorders3DModel(buildingGeometry,...
    xBeamHingeObjects,zBeamHingeObjects,BuildingModelDirectory,AnalysisType)
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
    ...
    fid = fopen('DefineBeamHingeRecorders3DModel.tcl','wt');
    ...
    % Defining recorders for Z-Direction beam hinge elements
    fprintf(fid,'%s\n','# Z-Direction beam hinge element force recorders');
    % Looping over all stories
    for i = 1:buildingGeometry.numberOfStories 
        fprintf(fid,'%s\t','recorder Element -file'); 
        fprintf(fid,'%s\t',strcat('ZBeamHingeForcesLevel',...
            num2str(i + 1),'.out'));
        fprintf(fid,'%s\t','-time -ele');     
    ...
    % Defining recorders for Z-Direction beam hinge elements
    fprintf(fid,'%s\n','# Z-Direction beam hinge element deformation recorders');
    % Looping over all stories
    for i = 1:buildingGeometry.numberOfStories 
        fprintf(fid,'%s\t','recorder Element -file'); 
        fprintf(fid,'%s\t',strcat('ZBeamHingeDeformationsLevel',...
            num2str(i + 1),'.out'));
        fprintf(fid,'%s\t','-time -ele');     
        % Looping over the number of Z-direction bays
        for j = 1:(buildingGeometry.numberOfZBays)
