...
buildingIDLIST = PHRInputs.BldgIdLIST;
numBldgs = length(buildingIDLIST);
deltaYieldBasis = PHRInputs.deltaYieldBasis;

% PL_LIST = [0.04, 0.02];
PL_LIST = 0.02;
roofDR_PL = zeros(numBldgs, length(PL_LIST)); 
V_perfLimit = zeros(numBldgs, length(PL_LIST));
delta_y_eff = zeros(numBldgs, 1);
Vyield = zeros(numBldgs, 1); 
delta_u = zeros(numBldgs, 1);
deltaForVbmax = zeros(numBldgs, 1); 
mu = zeros(numBldgs, length(PL_LIST)); 
Vb_max = zeros(numBldgs, 1);
