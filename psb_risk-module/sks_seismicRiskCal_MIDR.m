...
factorOnImMin =        MIDRInputs.factorOnImMin;
verbose =              MIDRInputs.verbose;
...
%% X. im bound
% %  X.0. factorOnImMin kicks in only if lowerBound = 99, i.e. when imMin is used.  Since, it's possible that (true imMin) < (imMin obtained from analysis). 
% %  X.1. (UNBOUNDED CASE) 
...
% %  X.2. (LOWER-BOUNDED CASE) 
    ...
    lowerBoundLIST =  0.01; % assign 99 to indicate that lowerBound is taken as the minimum im Value from fragility analysis
    upperBoundLIST = 5; % lower bound bound (over im or afe, as may be the case)
