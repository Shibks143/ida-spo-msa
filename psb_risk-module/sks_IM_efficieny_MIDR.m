...
for i = 1:size(BldgIdAndZoneLIST, 1) % for each building
    ...
    if isSinglePeriodAnalysis
        betaRTRCtrlMin(i,:) = betaRTRCtrl;
        muCtrlEff(i,:)      = muCtrl;
        timePEff(i,:)       = timePLIST;
    ...
    if doPlotFragMedian == 1 && ~isSinglePeriodAnalysis
        figure(i); hold on; grid on
        for k = 1:size(dsToPlotFragParam, 2)
            styleID = mod(k-1, length(lineStyleList)) + 1;
            plot(timePLIST, muCtrl(:, k), lineStyleList{styleID}, 'LineWidth', 1.5);
        end
    ...
    if doPlotFragDispers == 1 && ~isSinglePeriodAnalysis
        figure(100+i); hold on; grid on
        idx = timePLIST <= 3.0;
        for k = 1:size(dsToPlotFragParam, 2)
            styleID = mod(k-1, length(lineStyleList)) + 1;
            plot(timePLIST(idx), betaRTRCtrl(idx, k), lineStyleList{styleID}, 'LineWidth', 1.5);
        end
    ...
    if doPlotFragBound == 1 && ~isSinglePeriodAnalysis
...
if isSinglePeriodAnalysis
    Tsingle = table(bldgIdLIST, timePEff, muCtrlEff, betaRTRCtrlMin);
    disp(Tsingle)
else
    Togm = prod(timePEff, 2).^(1/size(timePEff,2));
    Tmulti = table(bldgIdLIST, timePEff, muCtrlEff, betaRTRCtrlMin, Togm);
    disp(Tmulti)
end
...
% Final fragility curves only for single-period analysis
if isSinglePeriodAnalysis
    sks_fragilityCurvesBasedOnMIDRDamageStates(muCtrlEff, betaRTRCtrlMin, dsToPlotFragParam, damageMeasure, intensityMeasureType, baseFolder)
end