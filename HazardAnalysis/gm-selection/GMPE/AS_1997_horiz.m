function [sa, sigma] = AS_1997_horiz(M, r_rup, T, is_soil, fault_type, HW, arb)

% by Jack Baker, 2/1/05, modified 1/12/2010
% Stanford University
% bakerjw@stanford.edu
%
% Compute the Abrahamson and Silva ground motion prediction (1997
...
if (nargin < 7) % arbitrary/average component not supplied
    arb = 1;
end
...
if (length(find(period == T)) == 0)
    ...
    sigma = abrahamson_silva_sigma(M, index, arb);
end
...
function [sigma] = abrahamson_silva_sigma(M, index, arb)
% calculate the sigma
...
if (arb) % inflate the sigma using a functional fit developed by Baker and Cornell (2006)
    sigma = sigma * sqrt(2/(1.78-0.039*log(period(index))));
end