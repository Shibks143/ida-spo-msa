function [median, sigma] = AB_2006_ENA(M, T, Rcd, stress, Vs30)
% by Lynne Schleiffarth, 1/25/10
% Stanford University
% lynne.schleiffarth@stanford.edu
%
%
% Atkinson and Boore 2006 GMPE, based on:
% "Earthquake Ground-Motion Prediction Equations for Eastern North America"
% by Gail M. Atkinson and David M. Boore (2006), 
% Bulletin of the Seismological Society of America v. 96; no. 6; p. 2181-2205
%
...
% otherwise, interpolate between coeffients
    ...
    delta = interp1([T_low T_high], [delta(ilow) delta(ihigh)], T);
    M1 = interp1([T_low T_high], [M1(ilow) M1(ihigh)], T);
    Mh = interp1([T_low T_high], [Mh(ilow) Mh(ihigh)], T);
end
...
ilow = min(find(period_soil<=T));
T_low = period_soil(ilow);
ihigh = max(find(period_soil>=T));
...
if Vs30>=2000 || Vs30==Vref
    ...
    % otherwise, interpolate between coefficients
        ...
        blin = interp1([T_low T_high], [blin(ilow) blin(ihigh)], T);
        b1 = interp1([T_low T_high], [b1(ilow) b1(ihigh)], T);
        b2 = interp1([T_low T_high], [b2(ilow) b2(ihigh)], T);
    end    
    % compute bnl from eqn. (8)
    if Vs30<=v1
        bnl = b1;
    elseif v1<Vs30 && Vs30<=v2
        bnl = (b1-b2)*log(Vs30/v2)/log(v1/v2);
    elseif v2<Vs30 && Vs30<=Vref
        bnl = b2*log(Vs30/Vref)/log(v2/Vref);
    else
        bnl = 0;
    end
    % compute S from eqn. (7)
    pgaBC = AB_2006_ENA(M, 0, Rcd, stress, Vref);
    if pgaBC<=60
        S = log10(exp(blin*log(Vs30/Vref) + bnl*log(60/100)));
    else
        S = log10(exp(blin*log(Vs30/Vref) + bnl*log(pgaBC/100)));
    end    
end
...
% eqn. (5)
logSa = c(1) + c(2).*M + c(3).*M.^2 + (c(4)+c(5).*M).*f1 + ...
    (c(6)+c(7).*M).*f2 + (c(8)+c(9).*M).*f0 + c(10).*Rcd + S;

%**************************************************************************
% COMPUTE STRESS ADJUSTMENT FACTOR USING EQN. (6)
%**************************************************************************
logSF2 = min((delta+0.05), (0.05 + delta*max(M-M1, 0)/(Mh-M1)));
...
%**************************************************************************
if T>=0
    median = (10.^logSa_adj)/981; % convert to units of g
else
    median = (10.^logSa_adj);
end
...
%**************************************************************************
sigma = 0.3 * log(10) * ones(size(median));
