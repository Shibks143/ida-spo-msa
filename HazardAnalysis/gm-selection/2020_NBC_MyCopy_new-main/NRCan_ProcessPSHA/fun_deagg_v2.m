...
function fun_deagg_v2(oqOutpDirDis, oqOutpDirHaz, lonLat, imString, returnP, deaggType, doSave)
% clear; 
tic; %close all; 
...
dirFig = 'Output_Van_Haz_figs';
extensions = {'fig', 'epsc', 'jpeg'}; % {'fig', 'epsc', 'png', 'jpeg', 'emf'};
...
cd(baseFolder); % back to the base directory for hazard processing
[imVals, afeVals, poeVals50y] = fun_returnHazCurFromPSHAFiles_v2(fullOqOutpDirHaz, lonLat, imString);


% For longer period, we have repeated zeros at the end; throwing interp1 off; 
% using unique values to inerpolate now. 
% saT1Tr = interp1(poeVals50y, imVals, poeT, 'pchip');
[~, ind] = unique(afeVals); 
...
%     figure('units','normalized','position',[0.25 0.25 0.45 0.50])
    ...
    tol = 0.05;     cd(baseFolder); % remove contributions less than 0.1%
...
%     remove_empty_bars(h, tol);
    ...
    legh = legend(hBarForLeg, legList, 'Interpreter', 'latex'); %hold off;
...
% move and rotate axis labels depending on az, el, and pbaspect
    ...
    elseif abs(sum(azNew - (-60))) + abs (elNew - 20) < 1e-2 && sum(abs(pbaspect - [2.4 4.5 1])) < 1e-2
        set(hx, 'Position', posx.*[1.1,1,1],'Rotation', 25); 
        set(hy, 'Position', posy.*[1.2,0.8,1.1],'Rotation', -35);
    end
    
    zMax = zlim; zMax = zMax(2); zMax = 5*ceil(zMax/5);
    zlim([0 zMax]);
    htext = text(55, 4, 0.9 * zMax, textStr, 'Interpreter', 'latex', 'FontSize', 14, 'FontWeight', 'bold', 'HorizontalAlignment','left', 'EdgeColor', 'k');
