...
def write_oq_sourcefile(model, modelpath, logicpath, multimods):
    ...
    for m in model:
        ...
        # get tectonic region
        if m['trt'] == 'Wcrust_med_clC.txt':
            tectonic_reg = 'Active Shallow Crust'
        elif m['trt'] == 'WinterfaceCombo_medclC.txt':
            tectonic_reg = 'Subduction Interface'
        elif m['trt'] == 'ENA_med_clC.txt':
            tectonic_reg = 'Stable Shallow Crust'
        elif m['trt'] == 'WcrustFRjb_med_clC.txt':
            tectonic_reg = 'Active Shallow Fault'
        elif m['trt'] == 'Woffshore_med_clC.txt':
            tectonic_reg = 'Active Shallow Offshore'
        elif m['trt'] == 'WinslabD30_med_clC.txt':
            tectonic_reg = 'Subduction IntraSlab30'
        elif m['trt'] == 'WinslabD50_med_clC.txt':
    ...
    outxml = path.join(modelpath, ''.join((outbase, '_source_model_logic_tree_FF.xml')))
    f = open(outxml,'w')
    f.write(newxml)
    f.close()
