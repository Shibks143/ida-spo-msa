    ...
    if((isConv == 1) | (includeNonConvResults == 1))
        ...
        %   Some of the data are from the file that 
            ...
            % Write information for structure E...This doesn't write a new record each time, but only for each record
                ...
                E{eqRecordTableIndex}.RID    = eqNum;        
                E{eqRecordTableIndex}.RCompXID    = eqNum * 10 + 1;         % ADDED for GeoMean Analysis     
                E{eqRecordTableIndex}.RCompYID    = eqNum * 10 + 2;         % ADDED for GeoMean Analysis
                E{eqRecordTableIndex}.RecX   = EQLIST{eqNum * 10 + 1}.File;
                E{eqRecordTableIndex}.RecY   = EQLIST{eqNum * 10 + 2}.File; % ADDED for GeoMean Analysis
