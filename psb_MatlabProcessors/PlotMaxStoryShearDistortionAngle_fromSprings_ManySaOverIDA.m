...
%
%   Need to plot the IDA first b/c this needs the file that the IDA plotter
%   creates.
%
...
for eqIndex = 1:length(eqNumberLIST)
    ...
    for saIndex = 2:length(saTOneForRunLIST)
        ...
        if(saTOneForRun < maxSaToPlot_errorCheck)
            ...
            figure(eqIndex)
            hold on
            PlotMaxStoryShearDistortionAngle_fromSprings_proc(analysisType, eqNumber, saTOneForRun, lineWidth, markerType);

        end
    end
    % Format 
    temp = sprintf('Model: %s, EQ: %d', modelNameForTitle, eqNumber);
    title(temp)
    hold off
end
