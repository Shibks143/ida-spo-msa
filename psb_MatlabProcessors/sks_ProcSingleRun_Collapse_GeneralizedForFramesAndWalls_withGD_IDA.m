% Procedure: sks_ProcSingleRun_Collapse_GeneralizedForFramesAndWalls_withGD_IDA 
% -------------------
%
...
%
% -------------------
function [scaleFactorForRun, maxDriftRatioForFullStr, maxResidualDriftRatioForFullStr, maxPFAForFullStr, isNonConv, isSingular, isCollapsed] = ...
...
% Finally maxDriftRatioForFullStr = maxDriftForFullFrame
...
%%%%% Compute Max Interstory Residual Drift Ratio for the Full Building %%%

% added (on 13-sept-2026) for RDR-based IDA curves. Uses the same max-over-stories
% based on the RESIDUAL (post-shaking) interstory drift.
maxResidualDriftRatioForFullStr = 0;
for i = 1:length(storyDriftRatioToSave)
    currentResidualDriftRatio = storyDriftRatioToSave{i}.ResidualAbs;
    if(currentResidualDriftRatio > maxResidualDriftRatioForFullStr)
        maxResidualDriftRatioForFullStr = currentResidualDriftRatio;
    end
end

%%%%%%%%% End of Computation of Max Residual Drift Ratio %%%%%%%%%%%%%%%%%%%


%%%%% Compute Max Peak Floor Accleration for the Full Building %%%%%%%%%%%%

maxPFAForFullStr = 0;
for floorNum = 1:numFloors
    currentPFA = floorAccelToSave{floorNum}.absAbsMaxUnfiltered / g;
    if(currentPFA > maxPFAForFullStr)
        maxPFAForFullStr = currentPFA;
    end
end



%%%%%%%%% End of Computation of Max Peak Floor Accleration %%%%%%%%%%%%%%%%
...
if(dataSavingOption == 2)
    ...
    save(fileName, 'eqNumber', 'scaleFactorForRun', 'saTOneForRun', 'PGA', 'saCompScaled', 'saGeoMeanScaled','floorAccelToSave', 'storyDriftRatioToSave', ...
        'roofDriftRatioToSave', 'maxDriftForFullFrame', 'maxDriftRatioForFullStr', 'maxResidualDriftRatioForFullStr', 'maxPFAForFullStr', 'absMaxDisplOfFloor', ...
        'buildingHeight', 'numStories', 'maxConvergedTime', 'eqTimeLength', 'isConvForFullEQ', ...
