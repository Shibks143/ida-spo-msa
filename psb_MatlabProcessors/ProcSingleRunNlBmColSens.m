%
% Procedure: ProcSingleRunNlBmColSens_NoFilt.m
% -------------------
%
% Without floor acceleration filter (to remove numerical noise)!
%
...
%
% -------------------
% function[void] = ProcessSingleRun(analysisType, modelName, saTOneForRun, eqNumber)
...
% % Input the node used to record the input acceleration, to calc absolute accel. from relative
% baseNodeNum = 7;  % NOT used b/c I forgot to put a mass at the node, so the accel response is strange!
...
%%%%%%%%%%%%%%%%%%
% Old way to compute absolute accelerations, using the input accel TH - still used for now
%%%%%%%%%%%%%%%%%%   
...
for floorNum = 2:length(floorHeightsLIST)
    ...
    % Compute the residual drift ratios.  Take the average of the nax and min drifts over the last few seconds of the EQ.
        ...
        % Change the range if the range is negative
            if(pointNumAtStartOfResidualCalc < 5)
                pointNumAtStartOfResidualCalc = 5;
            end
...
for floorNum = 2:numFloors
    ...
    %   Note that the ComputeAllResponsesNewmark.m returns more information than I need (i.e. displ., vel., etc.), so I am storing those in junk variables!
        %function[maxRelDispReponse, maxRelVelReponse, maxAbsAccelReponse,
        %maxPsuedoAccelReponse] = ComputeAllResponsesNewmark(accelTH,
        %timeVector, dT, period, dampRatio)
        
        % To do this, I need to go back to the MatlabProcessors folder, do
        % the processing, then come back to this folder
...
% Find the maximum time for the EQ (get info from file)
% defineEQInfoForMATLAB;     % We already opened this earlier in this file,
% so we don't need to here!
eqTimeLength = dtForEQRecord(eqNumber) * numPointsForEQRecord(eqNumber);
...
% Compute Plastic hinge rotations for all elements (in the 1-36 numbering)
    ...
    % NOTICE: I used to add the bond-slip PHR to the element PHR to get the total, but now I don't let the bond spring yield, so I don't do that.  The old processor
    %   that does that is in the backup folder dated 12-15-04.
    
    
    
        % Input all of the element level things
            % For all elements that have the same numbering (i.e. no splice), just copy the data.
                for eleIndexNum = 1:36
        ...
        %   Also, save the max for each element (total PHR).
                ...
                for eleNum = 1:36
                    ...
                    % Make the total PHR be just from the element (i.e. no bond-slip contribution)
                    elementCombinedPHR{eleNum}.endi.eleTotalTH = elementCombinedPHR{eleNum}.endi.eleTH;
                    elementCombinedPHR{eleNum}.endj.eleTotalTH = elementCombinedPHR{eleNum}.endj.eleTH;               
...
save(fileName, 'absMaxPHRToSave', 'floorAccelToSave', 'storyDriftRatioToSave', 'maxConvergedTime', 'eqTimeLength', 'maxColPHR', 'maxBmPHR', 'colWithMaxPHR',...
    'bmWithMaxPHR', 'eqNum', 'scaleFactorForRun', 'saValue', 'isCurrentAnalysisConv', 'maxTolUsed', 'PGA')
