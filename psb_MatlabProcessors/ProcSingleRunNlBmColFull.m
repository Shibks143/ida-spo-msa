%
% Procedure: ProcSingleRunNlBmColFull.m
% -------------------
%
% Same as ProcSingleRunNlBmColSens.m, but this saves more data - for later plotting, etc..
%
...
%
%           - NOTICE - nodeNumsAtEachFloorLIST is hard-coded right now!!!
%
%
%
%
...
%
% -------------------
% function[void] = ProcSingleRunNlBmColFull(analysisType, modelName, saTOneForRun, eqNumber)
...
% Input the paramters used to filter the floor acceleration, using Newmark integration.
    ...
    dampRatio = 0.01;    % To not make the value artificially high
...
% Compute Plastic hinge rotations for all elements (in the 1-36 numbering)
    ...
    %   - subFields: .bondTH, .bondAbsMax, .eleTH, .eleAbsMax, .combinedTH, .combinedAbsMax
        ...
        % Input all of the element level things
            % For all elements that have the same numbering (i.e. no splice), just copy the data.
                for eleIndexNum = 1:9
                    elementCombinedPHR{eleIndexNum}.endi.eleTH = elementArray{eleIndexNum}.PHRTH(:,2);
                    elementCombinedPHR{eleIndexNum}.endj.eleTH = elementArray{eleIndexNum}.PHRTH(:,3);        
                end
                for eleIndexNum = 15:18
                    elementCombinedPHR{eleIndexNum}.endi.eleTH = elementArray{eleIndexNum}.PHRTH(:,2);
                    elementCombinedPHR{eleIndexNum}.endj.eleTH = elementArray{eleIndexNum}.PHRTH(:,3); 
                end
                for eleIndexNum = 24:36
                    elementCombinedPHR{eleIndexNum}.endi.eleTH = elementArray{eleIndexNum}.PHRTH(:,2);
                    elementCombinedPHR{eleIndexNum}.endj.eleTH = elementArray{eleIndexNum}.PHRTH(:,3); 
                end
                
            % Get results for the elements were the numbers need to be changed
                % Story 2 columns
                elementCombinedPHR{10}.endi.eleTH = elementArray{1001}.PHRTH(:,2);
                elementCombinedPHR{10}.endj.eleTH = elementArray{1002}.PHRTH(:,3);        

                elementCombinedPHR{11}.endi.eleTH = elementArray{1101}.PHRTH(:,2);
                elementCombinedPHR{11}.endj.eleTH = elementArray{1102}.PHRTH(:,3);  
                
                elementCombinedPHR{12}.endi.eleTH = elementArray{1201}.PHRTH(:,2);
                elementCombinedPHR{12}.endj.eleTH = elementArray{1202}.PHRTH(:,3);  

                elementCombinedPHR{13}.endi.eleTH = elementArray{1301}.PHRTH(:,2);
                elementCombinedPHR{13}.endj.eleTH = elementArray{1302}.PHRTH(:,3);  

                elementCombinedPHR{14}.endi.eleTH = elementArray{1401}.PHRTH(:,2);
                elementCombinedPHR{14}.endj.eleTH = elementArray{1402}.PHRTH(:,3);  

                % Story 3 columns
                elementCombinedPHR{19}.endi.eleTH = elementArray{1901}.PHRTH(:,2);
                elementCombinedPHR{19}.endj.eleTH = elementArray{1902}.PHRTH(:,3);        

                elementCombinedPHR{20}.endi.eleTH = elementArray{2001}.PHRTH(:,2);
                elementCombinedPHR{20}.endj.eleTH = elementArray{2002}.PHRTH(:,3);  
                
                elementCombinedPHR{21}.endi.eleTH = elementArray{2101}.PHRTH(:,2);
                elementCombinedPHR{21}.endj.eleTH = elementArray{2102}.PHRTH(:,3);  
                
                elementCombinedPHR{22}.endi.eleTH = elementArray{2201}.PHRTH(:,2);
                elementCombinedPHR{22}.endj.eleTH = elementArray{2202}.PHRTH(:,3);  

                elementCombinedPHR{23}.endi.eleTH = elementArray{2301}.PHRTH(:,2);
                elementCombinedPHR{23}.endj.eleTH = elementArray{2302}.PHRTH(:,3);  

        % Now get all of the bond slip plastic rotation information - for the base columns, we should get the base bond spring information, but
        %   I forgot to record the response of these springs, so rather than rerunning the analyses, I will compute the ratio in stiffness between
        %   the bond and element yielded flexibilities to find a factor by which to multiply the element PHR to get an estimate for the total element + 
        %   bond PHR.
                thLength = length(elementArray{621}.JointForceAndDeformation(:,1)); % For use with zeros
                
                
                % Input the factor by which to multiply the element PHR to estimate the bond-slip deformation.
                %   This is done b/c I forgot to record the base spring bond-slip deformations.  See hand notes dated 9-27-04.
                %   The negative is b/c of the reversed sign later when the PHR's are added together.
                factor = -1.0;
                
                
                % Story 1 columns
                elementCombinedPHR{1}.endi.bondDisplTH = factor * elementArray{1}.PHRTH(:,2);
                elementCombinedPHR{1}.endj.bondDisplTH = elementArray{621}.JointForceAndDeformation(:,1);

                elementCombinedPHR{2}.endi.bondDisplTH = factor * elementArray{2}.PHRTH(:,2);
                elementCombinedPHR{2}.endj.bondDisplTH = elementArray{622}.JointForceAndDeformation(:,1);
                
                elementCombinedPHR{3}.endi.bondDisplTH = factor * elementArray{3}.PHRTH(:,2);
                elementCombinedPHR{3}.endj.bondDisplTH = elementArray{623}.JointForceAndDeformation(:,1);
                
                elementCombinedPHR{4}.endi.bondDisplTH = factor * elementArray{4}.PHRTH(:,2);
                elementCombinedPHR{4}.endj.bondDisplTH = elementArray{624}.JointForceAndDeformation(:,1);

                elementCombinedPHR{5}.endi.bondDisplTH = factor * elementArray{5}.PHRTH(:,2);
                elementCombinedPHR{5}.endj.bondDisplTH = elementArray{625}.JointForceAndDeformation(:,1);

                % Story 2 columns
                elementCombinedPHR{10}.endi.bondDisplTH = elementArray{621}.JointForceAndDeformation(:,3);
                elementCombinedPHR{10}.endj.bondDisplTH = elementArray{631}.JointForceAndDeformation(:,1);

                elementCombinedPHR{11}.endi.bondDisplTH = elementArray{622}.JointForceAndDeformation(:,3);
                elementCombinedPHR{11}.endj.bondDisplTH = elementArray{632}.JointForceAndDeformation(:,1);
                
                elementCombinedPHR{12}.endi.bondDisplTH = elementArray{623}.JointForceAndDeformation(:,3);
                elementCombinedPHR{12}.endj.bondDisplTH = elementArray{633}.JointForceAndDeformation(:,1);
                
                elementCombinedPHR{13}.endi.bondDisplTH = elementArray{624}.JointForceAndDeformation(:,3);
                elementCombinedPHR{13}.endj.bondDisplTH = elementArray{634}.JointForceAndDeformation(:,1);

                elementCombinedPHR{14}.endi.bondDisplTH = elementArray{625}.JointForceAndDeformation(:,3);
                elementCombinedPHR{14}.endj.bondDisplTH = elementArray{635}.JointForceAndDeformation(:,1);

                % Story 3 columns
                elementCombinedPHR{19}.endi.bondDisplTH = elementArray{631}.JointForceAndDeformation(:,3);
                elementCombinedPHR{19}.endj.bondDisplTH = elementArray{641}.JointForceAndDeformation(:,1);

                elementCombinedPHR{20}.endi.bondDisplTH = elementArray{632}.JointForceAndDeformation(:,3);
                elementCombinedPHR{20}.endj.bondDisplTH = elementArray{642}.JointForceAndDeformation(:,1);
                
                elementCombinedPHR{21}.endi.bondDisplTH = elementArray{633}.JointForceAndDeformation(:,3);
                elementCombinedPHR{21}.endj.bondDisplTH = elementArray{643}.JointForceAndDeformation(:,1);
                
                elementCombinedPHR{22}.endi.bondDisplTH = elementArray{634}.JointForceAndDeformation(:,3);
                elementCombinedPHR{22}.endj.bondDisplTH = elementArray{644}.JointForceAndDeformation(:,1);

                elementCombinedPHR{23}.endi.bondDisplTH = elementArray{635}.JointForceAndDeformation(:,3);
                elementCombinedPHR{23}.endj.bondDisplTH = elementArray{645}.JointForceAndDeformation(:,1);

                % Story 4 columns
                elementCombinedPHR{28}.endi.bondDisplTH = elementArray{641}.JointForceAndDeformation(:,3);
                elementCombinedPHR{28}.endj.bondDisplTH = elementArray{651}.JointForceAndDeformation(:,1);

                elementCombinedPHR{29}.endi.bondDisplTH = elementArray{642}.JointForceAndDeformation(:,3);
                elementCombinedPHR{29}.endj.bondDisplTH = elementArray{652}.JointForceAndDeformation(:,1);
                
                elementCombinedPHR{30}.endi.bondDisplTH = elementArray{643}.JointForceAndDeformation(:,3);
                elementCombinedPHR{30}.endj.bondDisplTH = elementArray{653}.JointForceAndDeformation(:,1);
                
                elementCombinedPHR{31}.endi.bondDisplTH = elementArray{644}.JointForceAndDeformation(:,3);
                elementCombinedPHR{31}.endj.bondDisplTH = elementArray{654}.JointForceAndDeformation(:,1);

                elementCombinedPHR{32}.endi.bondDisplTH = elementArray{645}.JointForceAndDeformation(:,3);
                elementCombinedPHR{32}.endj.bondDisplTH = elementArray{655}.JointForceAndDeformation(:,1);

                % Floor 2 beams
                elementCombinedPHR{6}.endi.bondDisplTH = elementArray{621}.JointForceAndDeformation(:,2);
                elementCombinedPHR{6}.endj.bondDisplTH = elementArray{622}.JointForceAndDeformation(:,4);

                elementCombinedPHR{7}.endi.bondDisplTH = elementArray{622}.JointForceAndDeformation(:,2);
                elementCombinedPHR{7}.endj.bondDisplTH = elementArray{623}.JointForceAndDeformation(:,4);

                elementCombinedPHR{8}.endi.bondDisplTH = elementArray{623}.JointForceAndDeformation(:,2);
                elementCombinedPHR{8}.endj.bondDisplTH = elementArray{624}.JointForceAndDeformation(:,4);
           
                elementCombinedPHR{9}.endi.bondDisplTH = elementArray{624}.JointForceAndDeformation(:,2);
                elementCombinedPHR{9}.endj.bondDisplTH = elementArray{625}.JointForceAndDeformation(:,4);

                % Floor 3 beams
                elementCombinedPHR{15}.endi.bondDisplTH = elementArray{631}.JointForceAndDeformation(:,2);
                elementCombinedPHR{15}.endj.bondDisplTH = elementArray{632}.JointForceAndDeformation(:,4);

                elementCombinedPHR{16}.endi.bondDisplTH = elementArray{632}.JointForceAndDeformation(:,2);
                elementCombinedPHR{16}.endj.bondDisplTH = elementArray{633}.JointForceAndDeformation(:,4);

                elementCombinedPHR{17}.endi.bondDisplTH = elementArray{633}.JointForceAndDeformation(:,2);
                elementCombinedPHR{17}.endj.bondDisplTH = elementArray{634}.JointForceAndDeformation(:,4);
           
                elementCombinedPHR{18}.endi.bondDisplTH = elementArray{634}.JointForceAndDeformation(:,2);
                elementCombinedPHR{18}.endj.bondDisplTH = elementArray{635}.JointForceAndDeformation(:,4);        
                
                % Floor 4 beams
                elementCombinedPHR{24}.endi.bondDisplTH = elementArray{641}.JointForceAndDeformation(:,2);
                elementCombinedPHR{24}.endj.bondDisplTH = elementArray{642}.JointForceAndDeformation(:,4);

                elementCombinedPHR{25}.endi.bondDisplTH = elementArray{642}.JointForceAndDeformation(:,2);
                elementCombinedPHR{25}.endj.bondDisplTH = elementArray{643}.JointForceAndDeformation(:,4);

                elementCombinedPHR{26}.endi.bondDisplTH = elementArray{643}.JointForceAndDeformation(:,2);
                elementCombinedPHR{26}.endj.bondDisplTH = elementArray{644}.JointForceAndDeformation(:,4);
           
                elementCombinedPHR{27}.endi.bondDisplTH = elementArray{644}.JointForceAndDeformation(:,2);
                elementCombinedPHR{27}.endj.bondDisplTH = elementArray{645}.JointForceAndDeformation(:,4);           
                
                % Floor 5 beams
                elementCombinedPHR{33}.endi.bondDisplTH = elementArray{651}.JointForceAndDeformation(:,2);
                elementCombinedPHR{33}.endj.bondDisplTH = elementArray{652}.JointForceAndDeformation(:,4);
                
                elementCombinedPHR{34}.endi.bondDisplTH = elementArray{652}.JointForceAndDeformation(:,2);
                elementCombinedPHR{34}.endj.bondDisplTH = elementArray{653}.JointForceAndDeformation(:,4);

                elementCombinedPHR{35}.endi.bondDisplTH = elementArray{653}.JointForceAndDeformation(:,2);
                elementCombinedPHR{35}.endj.bondDisplTH = elementArray{654}.JointForceAndDeformation(:,4);
           
                elementCombinedPHR{36}.endi.bondDisplTH = elementArray{654}.JointForceAndDeformation(:,2);
                elementCombinedPHR{36}.endj.bondDisplTH = elementArray{655}.JointForceAndDeformation(:,4);   
        ...
        %   Also, save the max for each element (total PHR).
                ...
                for eleNum = 1:36
                    ...
                    % Element PHR
                    elementCombinedPHR{eleNum}.endi.eleMin    = min(elementCombinedPHR{eleNum}.endi.eleTH);
                    elementCombinedPHR{eleNum}.endi.eleMax    = max(elementCombinedPHR{eleNum}.endi.eleTH);
                    elementCombinedPHR{eleNum}.endi.eleAbsMax = max(abs(elementCombinedPHR{eleNum}.endi.eleMin), abs(elementCombinedPHR{eleNum}.endi.eleMax));
                    
                    elementCombinedPHR{eleNum}.endj.eleMin    = min(elementCombinedPHR{eleNum}.endj.eleTH);
                    elementCombinedPHR{eleNum}.endj.eleMax    = max(elementCombinedPHR{eleNum}.endj.eleTH);
                    elementCombinedPHR{eleNum}.endj.eleAbsMax = max(abs(elementCombinedPHR{eleNum}.endj.eleMin), abs(elementCombinedPHR{eleNum}.endj.eleMax));
                    
                    % Bond-slip PHR
                    elementCombinedPHR{eleNum}.endi.bondDisplMin    = min(elementCombinedPHR{eleNum}.endi.bondDisplTH);
                    elementCombinedPHR{eleNum}.endi.bondDisplMax    = max(elementCombinedPHR{eleNum}.endi.bondDisplTH);
                    elementCombinedPHR{eleNum}.endi.bondDisplAbsMax = max(abs(elementCombinedPHR{eleNum}.endi.bondDisplMin), abs(elementCombinedPHR{eleNum}.endi.bondDisplMax));
                    
                    elementCombinedPHR{eleNum}.endj.bondDisplMin    = min(elementCombinedPHR{eleNum}.endj.bondDisplTH);
                    elementCombinedPHR{eleNum}.endj.bondDisplMax    = max(elementCombinedPHR{eleNum}.endj.bondDisplTH);
                    elementCombinedPHR{eleNum}.endj.bondDisplAbsMax = max(abs(elementCombinedPHR{eleNum}.endj.bondDisplMin), abs(elementCombinedPHR{eleNum}.endj.bondDisplMax));
                                        
                    % Combine the bond + ele PHR
                    elementCombinedPHR{eleNum}.endi.eleTotalTH = elementCombinedPHR{eleNum}.endi.eleTH - elementCombinedPHR{eleNum}.endi.bondDisplTH;
                    elementCombinedPHR{eleNum}.endj.eleTotalTH = elementCombinedPHR{eleNum}.endj.eleTH - elementCombinedPHR{eleNum}.endj.bondDisplTH;                    
...
save(fileName, 'minutesToRunThisAnalysis', 'nodeArray', 'nodeNumToRecordLIST', 'elementNumToRecordLIST', 'elementArray',...
    'storyDriftRatio', 'floorAccel', 'elementCombinedPHR', 'absMaxPHRToSave', 'floorAccelToSave', 'storyDriftRatioToSave',...
    'maxConvergedTime', 'eqTimeLength', 'maxColPHR', 'maxBmPHR', 'colWithMaxPHR', 'bmWithMaxPHR', 'eqNum',...
    'scaleFactorForRun', 'saValue', 'isCurrentAnalysisConv', 'maxTolUsed', 'psuedoTimeVector', 'timeVectorProcessed',...
    'dtForAnalysis', 'PGA');
