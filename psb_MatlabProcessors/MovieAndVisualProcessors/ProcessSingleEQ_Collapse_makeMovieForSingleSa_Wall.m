...
%
% -------------------
function[] = ProcessSingleEQ_Collapse_makeMovieForSingleSa_Wall(bldgID, analysisType, modelName, eqNumber, saLevel, movieID, isEQOrPO)
...
% Now process the analysis at the Sa level at collapse.
    if (isEQOrPO == 1)
        ...
        % Process for a PO
    ProcSingleRun_Generalized_NlBmCol_PO_ForVisuals(analysisType, saLevel, eqNumber);                               % For PO analysis
    end
        
% Now go into the moview folder and create the movie for this EQ and Sa
% level
    buildingInfo = DefineInfoForBuildings;
    CreateBuildingMovie_Wall(analysisType, buildingInfo, bldgID, eqNumber, saLevel, movieID)
