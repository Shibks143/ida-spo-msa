% Procedure: ProcSingleRun_Collapse_GeneralizedForFramesAndWalls_Full_ForVisuals.m
% -------------------
%
...
%
% -------------------
function[void] = ProcSingleRun_Collapse_GeneralFramesAndWalls_ForVisuals(bldgID, analysisType, modelName, saTOneForRun, eqNumber)
...
% Get information in the RunInformation folder for this specific run
    ...
    runFolderName = 'RunInformation';
        ...
        % Added on 7-26-06 to plot these on the collapse mode figures
...
%         PGA = max(abs(max_temp), abs(min_temp));
        ...
        PGA = max(abs(inputGroundAcceleration));
...
%     (11-12-15, PSB) made the script a tad brief by replacing the above loop by following line
    timeVectorForEQRecord = currentTime : dtForCurrentEQ : currentTime + dtForCurrentEQ * length(inputGroundAcceleration);
...
%         disp('In element loop for EleGlobalTH');
    ...
    % Do not load this because it is not used anymore
    if(1==0)
    cd EleLocalTH;
...
% save(fileName, 'floorAccelToSave', 'storyDriftRatioToSave', 'maxConvergedTime', 'eqTimeLength',...
%     'eqNum', 'scaleFactorForRun', 'saValue', 'isCurrentAnalysisConv', 'maxTolUsed', 'maxDriftRatioForFullStr', 'PGA', 'absMaxDisplOfFloor',...
%     'saCompScaled', 'saGeoMeanScaled', 'nodeNumsAtEachFloorLIST', 'elementArray', 'nodeArray', 'storyDriftRatioToSave', 'numStories',...
%     'storyDriftRatio', 'periodUsedForScalingGroundMotionsFromMatlab')

% (11-12-15, PSB) removed storyDriftRatio from saving list
save(fileName, 'floorAccelToSave', 'storyDriftRatioToSave', 'maxConvergedTime', 'eqTimeLength',...
    'eqNum', 'scaleFactorForRun', 'saValue', 'isCurrentAnalysisConv', 'maxTolUsed', 'maxDriftRatioForFullStr', 'PGA', 'absMaxDisplOfFloor',...
    'saCompScaled', 'saGeoMeanScaled', 'nodeNumsAtEachFloorLIST', 'elementArray', 'nodeArray', 'storyDriftRatioToSave', 'numStories',...
