...
%
% -------------------
function[] = ProcessSingleEQ_Collapse_makeMovieForSingleSa_Frame_AndGD(bldgID, analysisType, modelName, eqNumber, saLevel, movieID, isEQOrPO, saDefType)
...
% Now process the analysis at the Sa level at collapse.
    if (isEQOrPO == 1)
        % Process for an EQ
        ProcSingleRun_Collapse_GeneralFramesAndWalls_ForVisualsGD(analysisType, modelName, saLevel, eqNumber)
        %ProcSingleRun_Collapse_GeneralizedForFramesAndWalls_withGD(bldgID, analysisType, modelName, saLevel, eqNumber);    % For EQ collapse analysis
    else
        error('Dont need one with ground disp for PO!'); 
        % Process for a PO
...
% level
    ...
    CreateBuildingMovie_Frame_AndGD(analysisType, buildingInfo, bldgID, eqNumber, saLevel, movieID, saDefType);
