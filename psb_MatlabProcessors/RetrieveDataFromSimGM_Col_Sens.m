...
% Retrieve the collapse Sa level for this component number (from the opened file)
      ...
      foundEQInList = 0;
      collapseSaLevel = -1;
      for colInd = 1:length(eqCompNumberLIST)
        tempEQNumInLIST = eqCompNumberLIST(colInd);
        if(tempEQNumInLIST == eqNumChosenComp)
            collapseSaLevel = collapseLevelForChosenComp(colInd);
            foundEQInList = 1;
        end
      end
...
%     disp('EQNum:')
%     disp(eqNum)
    temp = sprintf('EQNum: %d', eqNumChosenComp);
    disp(temp);
...
% Save the data from this run, ONLY if it's fully converged or the analysisOption defined in the "CreateFileForCalTech.m" file says to allow non-conv. results.
    ...
    %if((isConv == 1) | (includeNonConvResults == 1))
        ...
        eqNumBothComp = (eqNumChosenComp - mod(eqNumChosenComp, 10)) / 10;      % Just to convert the longer EQ comp. number back to the number that is general for both components
