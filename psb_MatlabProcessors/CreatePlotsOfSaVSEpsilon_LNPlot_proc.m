...
%
% -------------------
function[void] = CreatePlotsOfSaVSEpsilon_proc(analysisType, groundMotionSetUsed, periodUsedForScalingGroundMotions, dampingRatioUsedForSaDef)
...
% Make the figures and save them
    ...
    yLabel = sprintf('LN[Sa_{comp}(T_{1}=%.2fs)]', periodUsedForScalingGroundMotions);
    ...
    yLabel = sprintf('LN[Sa_{g.m.}(T_{1}=%.2fs)]', periodUsedForScalingGroundMotions);
    xLabel = sprintf('%s_{AS}(T_{1}=%.2fs)', epsilonString, periodUsedForScalingGroundMotions);
