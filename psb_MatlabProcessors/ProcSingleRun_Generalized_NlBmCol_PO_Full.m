% Procedure: ProcSingleRunLumpPlaSens.m
% -------------------
%
...
%
% -------------------
function[void] = ProcSingleRun_Generalized_NlBmCol_PO_Full(analysisType, saTOneForRun, eqNumber)
...
% Input the paramters used to filter the floor acceleration, using Newmark integration.
    ...
    dampRatio = 0.03;    % To not make the value artificially high
...
%         usingLeaningColumn = load('usingLeaningColumnOUT.out');
        ...
        try % (11-11-20, PSB) for diagrid model, joints may not be defined and this variable isn't writ to defineJointRecordersOUT.out
...
%     
    ...
    if hingeElementsToRecordLIST ~= -1
        cd Hinges;
...
%     colEleNum = columnNumsAtBaseLIST(colIndex);
%     baseShear.TH = baseShear.TH + elementArray{colEleNum}.globalForceTH(:, 1);
%     tempCurrentAbsMaxShearForEle = max(abs(max(elementArray{colEleNum}.globalForceTH(:, 1))), abs(min(elementArray{colEleNum}.globalForceTH(:, 1))));
%     displayTemp = sprintf('Column number %d, has an AbsMax shear of %.2f\n', colEleNum, tempCurrentAbsMaxShearForEle);
%     disp(displayTemp);
% end
% 
% baseShear.AbsMax = max(abs(max(baseShear.TH)), abs(min(baseShear.TH)));
