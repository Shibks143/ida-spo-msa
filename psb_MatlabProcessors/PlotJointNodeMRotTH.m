...
%     legend(legendName);
    grid on
    titleText = sprintf('Moment-Rotation of JointNode %d of Joint %d; Sa: %.2f; EQ: %.0f; Analysis: %s', jointNodeNum, jointNum, saTOneForRun, eqNumber, analysisType);
    title(titleText);
    yLabel = sprintf('Moment (k-inch)');
    hy = ylabel(yLabel);
    hx = xlabel('Rotation (radians)');
    FigureFormatScript
    hold off
