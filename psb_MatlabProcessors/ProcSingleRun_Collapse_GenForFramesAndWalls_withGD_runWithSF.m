% Procedure: ProcSingleRun_Collapse_GenForFramesAndWalls_withGD_runWithSF.m
% -------------------
%
% Same as ProcSingleRun_Collapse_GeneralizedForFramesAndWalls_withGD.m,
% except set up to process data for runs done with a given scale factor
% (rather than a given Sa level).
%
% ProcSingleRun_Collapse_GeneralizedForFramesAndWalls_withGD.m is same as the ProcSingleRunNlBmColSens.m, but imports that data from the output from the lumped plasticity model.
%
...
%                           Fields: TH, AbsMax
%       psuedoTimeVector - a 1 x numTimeSteps vector holding the
...
%
% -------------------
function[maxDriftRatioForFullStr, isNonConv, isSingular, isCollapsed] = ProcSingleRun_Collapse_GenForFramesAndWalls_withGD_runWithSF(analysisType, modelName, eqNumber, scaleFactorForRun, dataSavingOption)

% IMPORTANT: This was updated to process runs done with a scale factor
% rather than a spectral acceleration level (7-24-25-07, CBH)
...
% Go into the EQ and SF folders.  Get information in the RunInformation folder for this specific run
    ...
    %   either one or two decimal places.
        scaleFactorFolderOneDecimalPlace = sprintf('SF_%.1f', scaleFactorForRun);
        scaleFactorFolderTwoDecimalPlace = sprintf('SF_%.2f', scaleFactorForRun);
        scaleFactorFolderThreeDecimalPlace = sprintf('SF_%.3f', scaleFactorForRun);
        scaleFactorFolderFourDecimalPlace = sprintf('SF_%.4f', scaleFactorForRun);
        ...
        if(exist(scaleFactorFolderOneDecimalPlace) == 7)
            ...
            scaleFactorFolder = sprintf('SF_%.1f', scaleFactorForRun)
        elseif(exist(scaleFactorFolderTwoDecimalPlace) == 7)
            ...
            scaleFactorFolder = sprintf('SF_%.2f', scaleFactorForRun)
        elseif(exist(scaleFactorFolderThreeDecimalPlace) == 7)
            % If we get here, then the directory with two decimal places is correct.
            %disp('Using two decimal places for folder names...')
            scaleFactorFolder = sprintf('SF_%.3f', scaleFactorForRun)
        elseif(exist(scaleFactorFolderFourDecimalPlace) == 7)
            % If we get here, then the directory with two decimal places is correct.
            %disp('Using two decimal places for folder names...')
            scaleFactorFolder = sprintf('SF_%.4f', scaleFactorForRun)
        else
...
% For the PFA calculations, we need to get the EQ TH vector so that we can add it with the relative accelerations to get absolute accelerations at each floor.
    ...
    % Load the dT and number of points for this EQ (from txt files; NOT from the older .m file).
        ...
        eqTimeLength = load(numPointsFileName) * dtForCurrentEQ;
...
% Compute roof drift ratio.  This assumes that the displacement for drift is in dof 1 (x dof)
    ...
    roofDriftRatio.AbsMax = max(abs(max(roofDriftRatio.TH)), abs(min(roofDriftRatio.TH)));
...
if(dataSavingOption == 1 || dataSavingOption == 3)
    % Save the full data
    ...
    'eqNum', 'scaleFactorForRun', 'isCurrentAnalysisConv', 'isCollapsed', 'isSingular', 'maxTolUsed', 'maxDriftRatioForFullStr', 'PGA', 'absMaxDisplOfFloor',...
    'nodeNumsAtEachFloorLIST', 'elementArray', 'nodeArray', 'ground', 'storyDriftRatioToSave', 'numStories',...
    'storyDriftRatio', 'ground', 'buildingHeight', 'jointForceAndDefAbsMaxToSave',...
    ...
    'jointShearDistortionAbsMaxPerFloor', 'roofDriftRatioToSave')
end
if(dataSavingOption == 2 || dataSavingOption == 3)
    % Save a reduced amount of data
    fileName = ['DATA_reducedSensDataForThisSingleRun.mat'];
    save(fileName, 'floorAccelToSave', 'storyDriftRatioToSave', 'maxConvergedTime', 'eqTimeLength',...
    'eqNum', 'scaleFactorForRun', 'isCurrentAnalysisConv', 'isCollapsed', 'isSingular', 'maxTolUsed', 'maxDriftRatioForFullStr', 'PGA', 'absMaxDisplOfFloor',...
    'buildingHeight', 'jointForceAndDefAbsMaxToSave',...
    'beamAbsMaxPHRForFullBldg', 'beamAbsMaxPHRPerFloor', 'columnAbsMaxPHRForFullBldg', 'columnAbsMaxPHRPerStory', 'jointShearDistortionAbsMaxForFullBldg',...
    'jointShearDistortionAbsMaxPerFloor', 'roofDriftRatioToSave')
end
