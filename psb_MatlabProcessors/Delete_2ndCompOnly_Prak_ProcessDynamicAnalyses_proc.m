...
% Do processing
    ...
    if(isProcessMultipleCollapseRuns == 1)
        ...
        Prak_ProcessMultipleCollapseRuns_Generalized_withGD(analysisTypeLIST, modelNameLIST, eqNumberLIST_forProcessing, collapseDriftThreshold, dataSavingOption);
        disp('Process Multiple Collapse Runs - DONE')
    end
    
    % Plot IDAs - both horizontal components
    if(isPlotCollapseIDAs == 1)
        Delete_2ndCompOnly_PlotCollapseIDAs(analysisTypeLIST, eqNumberLIST_forCollapseIDAs, eqListForCollapseIDAs_Name, markerTypeLine, markerTypeDot, isPlotIndividualPoints, collapseDriftThreshold, isConvertToSaKircher);
