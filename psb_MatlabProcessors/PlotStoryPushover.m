...
%
% -------------------
function[void] = PlotStoryPushover(storyNum, analysisType, eqNumber, saTOneForRun, ~, maxNumPoints, markerType, lineWidth, SPO_index)
...
% load('DATA_allDataForThisSingleRun.mat', 'currentStoryShearTH', 'storyDriftRatio', 'buildingHeight');
% load('DATA_reducedSensDataForThisSingleRun.mat');

% Do plot
     
    storyShearArray = -currentStoryShearTH{storyNum};
    ...
    length2 = length(storyShearArray);
...
% disp('Check1');
    ...
    plot(storyDriftArray(1:numDataPointsUsed), storyShearArray(1:numDataPointsUsed), markerType, 'LineWidth', lineWidth);
...
% disp('Check2');
    ...
    yLabel = sprintf('Story Shear (kips)');
    hy = ylabel(yLabel);
    ...
    % Save the plot
        ...
        exportName = sprintf('Pushover_%s_Story_%d_storyShear', analysisType, storyNum);
