...
% If we are in the range of periods used in hazard analysis:
%   - Linearly interpolate the hazard values if we are within the range of
%       periods used in the hazard analysis.  
%   - Also, linearly interpolate ko and k for
%       period; for T < Tmax (usually 2 seconds).
% If we are outside the range of periods used in hazard analysis:
%   - Interpolate the lambda-Sa value assuming a 1/T shape.  This should be
%       conservative.  When I did this for Site 1 and the archetype buildings,
%       the highest we needed to extrapolate to was 2.6 seconds, so this should
%       not be too far off.
%   - Just use the ko and k values at 2 seconds.  This 
%       is only used for approximation when Sa>2g and T is outside
%       hazard analysis range (usually > 2 sec), so this approxmation
%       is not very important.
%
% Sanity checks on this script, to test each of the four possible combinations of treating periods and Sa values:
%   Check 1: saLevel = 0.6, siteNum = 1, period = 0.48, results looks good.
%   Check 2: saLevel = 2.2, siteNum = 1, period = 1.0, results looks good.
%   Check 3: saLevel = 0.43, siteNum = 1, period = 2.2, results looks good.
%   Check 4: saLevel = 2.5, siteNum = 1, period = 2.2, results looks good.
%

    numPeriods = length(hazard_Periods);
    if(period <= max(hazard_Periods))
        
        % If we get here, we are for a period within the range of the
        % hazard analysis.  So, just do linear interpolation to find the
        % hazard values.
            ...
            periodIndex;
            pastPeriodIndex = periodIndex - 1;
            currentPeriodInLoop;
            pastPeriodInLoop;

            % Find the interpolation ratio between the bounding periods.
            interpRatio = (period - pastPeriodInLoop) / (currentPeriodInLoop - pastPeriodInLoop);
        
            % Find the bounding hazard values and do ineterpolation.
            hazard_LambdaSaTOne_below = hazard_Lambda_allSaTOneAllPeriods(:, pastPeriodIndex);
            hazard_LambdaSaTOne_below(1);
            hazard_LambdaSaTOne_above = hazard_Lambda_allSaTOneAllPeriods(:, periodIndex);
            hazard_LambdaSaTOne_above(1);
            hazard_LambdaSaTOne = hazard_LambdaSaTOne_below + interpRatio*(hazard_LambdaSaTOne_above - hazard_LambdaSaTOne_below) ; 
            hazard_LambdaSaTOne(1);
            
            % We are withing the period range used for the hazard
            % analysis, so we do not need to adjust the hazard Sa values.
            hazard_SaTOne_adjustedForLongPeriodIfNeeded = hazard_SaTOne;
 
        % Do similar linear interpolation for ko and k.  Find the bounding
        % values and do interpolation.  This uses the interpolation
        % information developed above.
            % ko
            ko_below = ko_allPeriods(pastPeriodIndex);
            ko_above = ko_allPeriods(periodIndex);
            ko = ko_below + interpRatio*(ko_above - ko_below);
            % k
            k_below = k_allPeriods(pastPeriodIndex);
            k_above = k_allPeriods(periodIndex);
            k = k_below + interpRatio*(k_above - k_below);

    else
        % If we get here, we are outside of the period range used for hazard analysis.
       
        % Extrapolate the hazard curve assuming a 1/T spectral shape from
        % T=Tmax (usually 2 seconds) to the period of interest.  To do this
        % adjustment, I am going to change the Sa values rather than the
        % hazard values.  I will assume that the hazard values are
        % constant (using the values for the highest period).  I will then
        % adjust the Sa values according to the 1/T shape.
            % Find highest period
            highestPeriodIndex = length(hazard_Periods);
            highestPeriod = hazard_Periods(highestPeriodIndex);
            
            % Get hazard for highest period and use this
            hazard_LambdaSaTOne = hazard_Lambda_allSaTOneAllPeriods(:, highestPeriodIndex);
            
            % Adjust the Sa values to account for the assumed 1/T spectral
            % shape from the highest period to the period of interest.
            adjustmentFactorForSa = (1/period) / (1/highestPeriod);
            hazard_SaTOne_adjustedForLongPeriodIfNeeded = adjustmentFactorForSa * hazard_SaTOne;
        ...
        %    hazard analysis range (usually > 2 sec), so this approxmation
        %    is not very important.
            ko = ko_allPeriods(highestPeriodIndex);
            k = k_allPeriods(highestPeriodIndex);
...
if(saLevel < max(hazard_SaTOne_adjustedForLongPeriodIfNeeded))
    
    % Use PCHIP to evaluate the hazard curve value at the desired Sa level
    hazardValue = pchip(hazard_SaTOne_adjustedForLongPeriodIfNeeded, hazard_LambdaSaTOne, saLevel);

    % Perturb the saLevel to find the slope of the hazard curve...
    saLevel_perturbed = saLevel * (1.0 + ratioOfSaForPertubation);
    hazardValue_perturbed = pchip(hazard_SaTOne_adjustedForLongPeriodIfNeeded, hazard_LambdaSaTOne, saLevel_perturbed);
