%
% Procedure: ProcessStripeStatistics.m
% -------------------
...
%               - .StDev - standard devation for all EQ responces
%
%       maxHingeRot - maximum PH rotation for any element (i.e. max. of all elements in frame) for all EQ's - fields are: 
%               - .allEQs(eqIndex) - this is the value for all EQ's of the stripe
%               - .Mean - mean for all EQ responces
%               - .StDev - standard devation for all EQ responces
%
%       hingeNumWithMaxRot - hinge number corresponding to the maximum PH rotation above - fields are: 
%               - .allEQs(eqIndex) - this is the value for all EQ's of the stripe
%
%
%
...
%             hingeNumWithMaxRot.allEQs = zeros(length(eqNumberToRunLIST), 1);
        ...
        for eqIndex = 1:length(eqNumberToRunLIST)
                ...
                % Copy the information that we need for this EQ run
                    ...
                    for floorNum = 2:maxFloorNum
                        maxFloorAccel{floorNum}.allEQs(eqIndex) = floorAccel{floorNum}.AbsMax(1);   % Added it for DOF 1 on 9-2-04
                    end
...
%             save(fileName, 'analysisType', 'maxStoryDriftRatio', 'maxFloorAccel', 'maxHingeRot', 'hingeNumWithMaxRot');
