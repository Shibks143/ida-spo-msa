...
%         stDevCollapseSaTOneControlComp stDevLnCollapseSaTOneControlComp
    ...
    % ALL COMPONENTS - altered on 9-16-05
    load DATA_collapse_CollapseSaAndStats.mat collapseLevelForAllComp...
...
% increasing
    ...
    collapseLevelForAllComp_sorted_normByMCE = collapseLevelForAllComp_sorted / MCE;
...
% Plot the empirical CDF
    ...
    if(plotIndividualEQPoints == 1)
        plot(collapseLevelForAllComp_sorted_normByMCE, cummulativeProbOfCollapseEmpirical, markerTypeForIndivColResults);
    end
...
% Plot the lognormal CDF or normal CDF with only RTR variability
    ...
    meanLnCollapseSaTOneAllComp = mean(log(collapseLevelForAllComp_sorted_normByMCE));
    stDevLnCollapseSaTOneAllComp = std(log(collapseLevelForAllComp_sorted_normByMCE));
...
% Do final plot details
    ...
    title(titleText)
    xText = sprintf('[Sa_{code}(T=1.0s)] / [MCE of %.2f]', MCE)
    hx = xlabel(xText)
