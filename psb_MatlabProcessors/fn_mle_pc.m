function [theta, beta] = sks_Mle_MSA(saLevelForEachRun, numOfGroundMotions, isCollapsedForEachRun)
...
x0 = [mean(log(saLevelForEachRun)), std(log(saLevelForEachRun))];

%% Run optimization
options = optimset('MaxFunEvals',1000, 'GradObj', 'off'); %maximum 1000 iterations, gradient of the function not provided
x = fminsearch(@mlefit, x0, options, numOfGroundMotions, isCollapsedForEachRun, saLevelForEachRun) ;
theta = exp(x(1)); % return theta in linear space
beta = x(2);

%% Objective function to be optimized
function [loglik] = mlefit(params, numOfGroundMotions, isCollapsedForEachRun, saLevelForEachRun)
...
if params(2) < 0
    ...
    % parameter estimates
    p = normcdf(log(saLevelForEachRun), (params(1)), params(2)); 
    ...
    likelihood = binopdf(isCollapsedForEachRun', numOfGroundMotions', p'); %
