...
%   This only does the plot for BJF if the period is <=2 seconds, because
%   the BJF attenuation is only value to 2 seconds.
...
%
% -------------------
function[void] = CreatePlotsOfSaVSEpsilon_proc_withOtherFitTypes(analysisType, groundMotionSetUsed, periodUsedForScalingGroundMotions, dampingRatioUsedForSaDef, numberOfBins)
...
% Go to the correct folder and open the file of collapse results
    ...
    cd(analysisType)
    % Open file
    fileName = sprintf('DATA_collapse_CollapseSaAndStats_%s_SaGeoMean.mat', groundMotionSetUsed);
    load(fileName);
...
% Make the figures and save them
    ...
    if(periodUsedForScalingGroundMotions < 2.0001)
        ...
        plotNamePrefix = sprintf('SaVSEpsilon_Sacomp_BJFEpsilon_withNewFitTypes_%s', analysisType);
        PlotSaVSEpsilon_proc_withOtherFitTypes(saCollapse_current, epsilonAtTOne_current, markerType_nonOutliers, markerSize_nonOutliers, lineWidthForDots, markerType_outliers, markerSize_outliers, epsilonStepSizeForPlotting, markerType_bestFit, lineWidthForBestFitLine, markerType_CI, lineWidthForCI, equationTextFontSize, orginLineType, orginLineSize, legendTextFontSizeInThisFile, plotNamePrefix, numEQComp, yLabel, xLabel, figNum, groundMotionSetUsed, numberOfBins);
    end
    ...
    xLabel = sprintf('%s_{AS}(T_{1}=%.2fs)', epsilonString, periodUsedForScalingGroundMotions);
    plotNamePrefix = sprintf('SaVSEpsilon_Sagm_ASEpsilon_withNewFitTypes_%s', analysisType);
    PlotSaVSEpsilon_proc_withOtherFitTypes(saCollapse_current, epsilonAtTOne_current, markerType_nonOutliers, markerSize_nonOutliers, lineWidthForDots, markerType_outliers, markerSize_outliers, epsilonStepSizeForPlotting, markerType_bestFit, lineWidthForBestFitLine, markerType_CI, lineWidthForCI, equationTextFontSize, orginLineType, orginLineSize, legendTextFontSizeInThisFile, plotNamePrefix, numEQComp, yLabel, xLabel, figNum, groundMotionSetUsed, numberOfBins);
    ...
    if(periodUsedForScalingGroundMotions < 2.0001)
        ...
        yLabel = sprintf('Sa_{g.m.}(T_{1}=%.2fs)', periodUsedForScalingGroundMotions);
        xLabel = sprintf('%s_{BJF}(T_{1}=%.2fs)', epsilonString, periodUsedForScalingGroundMotions);
        plotNamePrefix = sprintf('SaVSEpsilon_Sagm_BJFEpsilon_withNewFitTypes_%s', analysisType);
        PlotSaVSEpsilon_proc_withOtherFitTypes(saCollapse_current, epsilonAtTOne_current, markerType_nonOutliers, markerSize_nonOutliers, lineWidthForDots, markerType_outliers, markerSize_outliers, epsilonStepSizeForPlotting, markerType_bestFit, lineWidthForBestFitLine, markerType_CI, lineWidthForCI, equationTextFontSize, orginLineType, orginLineSize, legendTextFontSizeInThisFile, plotNamePrefix, numEQComp, yLabel, xLabel, figNum, groundMotionSetUsed, numberOfBins);
    end
