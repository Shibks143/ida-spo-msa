%
% Procedure: PlotCollapseEmpiricalCDFWithFits_allRDR_proc.m
% -------------------
% RDR analogue of PlotCollapseEmpiricalCDFWithFits_allMIDR_proc.m.
% Plots the EMPIRICAL CDF (+ lognormal fit) for EVERY rdrLevel
% (IO, LS, CP, Collapse, ...) using per-EQ Sa data at each RDR level.
...
% DATA_FragilityParams_*_RDR.mat file that was saved WITH the raw matrices
% for both component types (see the "Fragility Functions - compute +
% save only (Control + All Comp)" block added to the end of
% sks_PlotCollapseIDAs_singleAnaType_RDR.m):
%
...
% -------------------
function sks_PlotCollapseEmpiricalCDFWithFits_allRDR_proc(idaInputs)
...
load(fragFileName, 'rdrLevels', 'rdrLevelLabels', 'meanLnSaVals', 'stDevLnSaVals', 'saValsAtTargetDriftControlComp', 'meanLnSaValsAllComp', 'stDevLnSaValsAllComp', 'saValsAtTargetDriftAllComp');
...
for caseIdx = 1:length(componentCases)
    ...
    %%%%%%%%%%% Plot loop - one empirical CDF (+ fit) per RDR level %%%%%%
    for limitStateIdx = 1:numLimitStates
        ...
        if numEQs < 2
            warning('%s - RDR level %d (%s) has fewer than 2 valid points - skipping', thisCase.label, limitStateIdx, rdrLevelLabels{limitStateIdx});
            continue
        end
        ...
        % Text annotation (median Sa and beta_RTR) 
           if(addTextAnnotations == 1)
                ...
                text(xPosNorm, yPosNorm, ...
                    sprintf('$$\\hat{S}_{%s} = %5.2f\\mathrm{g},\\ \\beta_{RTR} = %5.2f$$', rdrLevelLabels{limitStateIdx}, ...
    ...
    if(isConvertToSaKircher == 0)
        exportName = sprintf('CollapseCDF_%s_SaGeoMean_RDR', thisCase.label);
    else
        exportName = sprintf('CollapseCDF_%s_SaATC63_RDR', thisCase.label);
    end
