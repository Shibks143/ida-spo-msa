...
% function[void] = ProcessMultipleRuns(analysisTypeLIST, modelNameLIST, eqNumberAllStripesLIST, saTOneForRunLIST, elementUsed)
    ...
    for analysisAndModelNum = 1:length(analysisTypeLIST)
        ...
        for saTOneForRunNum = 1:length(saTOneForRunLIST)
            ...
            eqNumberLIST = eqNumberAllStripesLISTGeoMean{eqStripeNum};
                ...
                for eqNumberNum = 1:length(eqNumberLIST)
                    ...
                    eqNumber = eqNumberLIST(eqNumberNum) * 10 + 1;
                    ...
                    if (elementUsed == 1)
                        ...
                        if (useProcessorWithFilt == 1)
                            disp('Using Processor with Acceleration Filter!')
                            ProcSingleRunNlBmColSens_WithFilt
                        else
                            ProcSingleRunNlBmColSens
                            %ProcSingleRunNlBmColFullPO_newGeneral; % Added on 11-03-05
                        end
                    elseif (elementUsed == 2)
                        disp('Using Lumped Plasticity Processor!')
                        if (useProcessorWithFilt == 1)
                            disp('Using Processor with Acceleration Filter!')
                            ProcSingleRunLumpPlaSens_WithFilt
                        else
                            ProcSingleRunLumpPlaSens
                            %ProcSingleRunLumpPlaSens_butSavesAll
                            %ProcSingleRunLumpPlaPO_Full
                        end
                    else
                    ...
                    eqNumber = eqNumberLIST(eqNumberNum) * 10 + 2;
