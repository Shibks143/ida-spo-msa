% Procedure: ProcSingleRunLumpPlaFull.m
% -------------------
%
% Same as the ProcSingleRunNlBmColSens.m, but imports that data from the output from the lumped plasticity model.
%
% Same as the ProcSingleRunLumpPlaSens.m, but this saves more data - for later plotting, etc..
%
%
...
%
% -------------------
function[void] = ProcSingleRunLumpPlaFull(analysisType, modelName, saTOneForRun, eqNumber)
...
% Get information in the RunInformation folder for this specific run
    ...
    runFolderName = 'RunInformation';
        ...
        %scaleFactorForRun = load('scaleFactorForRunOUT.out');   % Name changed in Aug-Sept 2005
        scaleFactorForRun = load('scaleFactorAppliedToCompOUT.out');    
...
%         eqFullLength = load('eqFullLengthOUT.out');
    ...
    % This is the folder for this analysis (this EQ and Sa level) - save this
    analysisOutputFolderPath = [pwd];
...
for floorNum = 2:length(floorHeightsLIST)
    ...
    temp = sprintf('***Max drift for full frame is: %.3f', maxDriftForFullFrame);
    disp(temp);
...
% I am not sure why the path is wrong, but make it go to the right folder
% to save the results
cd(analysisOutputFolderPath);
