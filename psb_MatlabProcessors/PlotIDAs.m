...
saTOneForStripeLIST = [0.10, 0.19, 0.26, 0.30, 0.44, 0.55, 0.82, 1.20];   % With 1.20g stripe!
% saTOneForStripeLIST = [0.10, 0.19, 0.26, 0.30, 0.44, 0.55, 0.82];   % No 1.20g stripe!   
% saTOneForStripeLIST = [0.82];   
% saTOneForStripeLIST = [0.19, 0.26, 0.55, 0.82, 1.20];   % Sens. set with 1.20g stripe!
...
modelVersionPrefix = '(DesID1_v.65)_';       % LATER, MAKE THIS AN INPUT PARAMETER
% modelVersionPrefix = '(DesID1_v.64noGFrm)_';       % LATER, MAKE THIS AN INPUT PARAMETER
% modelVersionPrefix = '(DesID1_v.63noGFrmConc01)_';       % LATER, MAKE THIS AN INPUT PARAMETER
% modelVersionPrefix = '(DesID1_v.63Elastic)_';       % LATER, MAKE THIS AN INPUT PARAMETER
% modelVersionPrefix = '(DesID2_v.5)_';       % LATER, MAKE THIS AN INPUT PARAMETER
% modelVersionPrefix = '(DesID3_v.9noGFrm)_';       % LATER, MAKE THIS AN INPUT PARAMETER
% modelVersionPrefix = '(DesID6_v.4)_';       % LATER, MAKE THIS AN INPUT PARAMETER
% modelVersionPrefix = '(DesID5_v.2)_';       % LATER, MAKE THIS AN INPUT PARAMETER
% modelVersionPrefix = '(DesID9_v.3noGFrm)_';       % LATER, MAKE THIS AN INPUT PARAMETER
% modelVersionPrefix = '(DesID10_v.3)_';       % LATER, MAKE THIS AN INPUT PARAMETER
...
% elementsUsedForModel = 'clough';           % LATER, MAKE THIS AN INPUT PARAMETER

variableName = 'AllVar';
variableValue = 'Mean';

% variableName = 'cappingRotationF';
% variableValue = 0.13;

% variableName = 'bondHardSlopeF';
% variableValue = 0.01;

% variableName = 'useCrackedConc';
% variableValue = '1.00';

% variableName = 'slabHingeCapRotF';
% variableValue = 0.48;
...
%         plotSingleEQPoints = 0;
    ...
    % Define if you want to plot the mean and mean +/- sigma lines
        plotMeanLine = 1;       % NOTE - these only use the conv EQs for the line plots
%         plotMeanLine = 0;
        plotVarLines = 1;       % NOTE - these only use the conv EQs for the line plots
%         plotVarLines = 0;
...
%         meanAnalysisFolder = sprintf('%s(%s)_(%.2f)_(%s)', modelVersionPrefix, variableName, variableValue, elementsUsedForModel);    % For the sensitivity runs
        ...
        % Make filename based on EQ set used
        ...
        maxAvgDriftRatioAllStripes{stripeIndex} = maxAvgDriftRatio;
        residualStoryDriftRatioAllStripes{stripeIndex} = residualStoryDriftRatioAbsVal; % Use ABS VAL; not signed value!
        residualStoryDriftRatioRatioAllStripes{stripeIndex} = residualStoryDriftRatioRatio; % This is the ratio of residual drift to PTD
                        ...
                        stripeIndex;
                            ...
                            for eqIndex = 1:length(maxStoryDriftRatioAllStripes{stripeIndex}{storyNum}.allEQs)
                                ...
                                if (isCurrentEQFullyConv == 1)
                                    ...
                                    markerType = nonConvPointMarkerType;

                                    % Plot the point, if told to
                    ...
                    % Now plot the mean IDA, the mean +/- one sigmaRTR IDA, and the mean +/- one sigmaTOTAL IDA
                        % Plot the mean IDA line (the mean responce of the AllVar=Mean run)
                            ...
                            for stripeIndex = 1:length(saTOneForStripeLIST)
                                    ...
                                    meanResponceForPlot(1, stripeIndex + 1) = exp(maxStoryDriftRatioAllStripes{stripeIndex}{storyNum}.MeanLnConvEQs); % This leaves 
                    ...
                    % Now plot the IDA lines for +/- 1.0 sigmaRTR
                        ...
                        % Compute +/- one sigma RTR IDA lines
                            ...
                            for stripeIndex = 1:length(saTOneForStripeLIST)
                                ...
                                % Mean minum sigma
                                    meanMinusSigmaRTRResponceForPlot(1, stripeIndex + 1) = exp(maxStoryDriftRatioAllStripes{stripeIndex}{storyNum}.MeanLnConvEQs - ...
                                        maxStoryDriftRatioAllStripes{stripeIndex}{storyNum}.StDevLnConvEQs);
                                    
                                % Mean minum sigma
                                    meanPlusSigmaRTRResponceForPlot(1, stripeIndex + 1) = exp(maxStoryDriftRatioAllStripes{stripeIndex}{storyNum}.MeanLnConvEQs + ...
                                        maxStoryDriftRatioAllStripes{stripeIndex}{storyNum}.StDevLnConvEQs);          
...
%                     legend(legendName);
                    ...
                    title(titleText);
                    yLabel = sprintf('Sa_{g.m.}(T=1sec) [g]');
                    hy = ylabel(yLabel);
                    xLabel = sprintf('Peak Story Drift: Story %d', storyNum);
                    hx = xlabel(xLabel);
                    box on
                    FigureFormatScript

                    % Save the current figure
                    plotName = sprintf('IDA_PeakTransientDrift_Story_%d.fig', storyNum);
                    hgsave(plotName)
                    exportName = sprintf('IDA_PeakTransientDrift_Story_%d.emf', storyNum);
                ...
                % Draw the IDA for each story drift responce
                    ...
                    for stripeIndex = 1:length(saTOneForStripeLIST)
                        ...
                        stripeIndex;
                            ...
                            for eqIndex = 1:length(maxAvgDriftRatioAllStripes{stripeIndex}.allEQs)
                                ...
                                if (isCurrentEQFullyConv == 1)
                                    ...
                                    % Plot the point
                                        plotVectorX = maxAvgDriftRatioAllStripes{stripeIndex}.allEQs(eqIndex);     % These are the individual EQ responces for the AllVar=Mean run.
                                        plotVectorY = saTOneForStripeLIST(stripeIndex);  % Just Sa level for stripe
                                        if (plotSingleEQPoints == 1)
                                            plot(plotVectorX, plotVectorY, markerType);
                                        end
                    ...
                    % Now plot the mean IDA, the mean +/- one sigmaRTR IDA, and the mean +/- one sigmaTOTAL IDA
                        % Plot the mean IDA line (the mean responce of the AllVar=Mean run)
                            ...
                            for stripeIndex = 1:length(saTOneForStripeLIST)
                                    ...
                                    meanResponceForPlot(1, stripeIndex + 1) = exp(maxAvgDriftRatioAllStripes{stripeIndex}.MeanLnConvEQs); % This leaves 
                    ...
                    % Now plot the IDA lines for +/- 1.0 sigmaRTR
                        ...
                        % Compute +/- one sigma RTR IDA lines
                            ...
                            for stripeIndex = 1:length(saTOneForStripeLIST)
                                ...
                                % Mean minum sigma - error corrected here
                                % on 9-20-05
                                    meanMinusSigmaRTRResponceForPlot(1, stripeIndex + 1) = exp(maxAvgDriftRatioAllStripes{stripeIndex}.MeanLnConvEQs - ...
                                        maxAvgDriftRatioAllStripes{stripeIndex}.StDevLnConvEQs);
                                    
                                % Mean minum sigma - error corrected here
                                % on 9-20-05
                                    meanPlusSigmaRTRResponceForPlot(1, stripeIndex + 1) = exp(maxAvgDriftRatioAllStripes{stripeIndex}.MeanLnConvEQs + ...
                                        maxAvgDriftRatioAllStripes{stripeIndex}.StDevLnConvEQs);          
...
%                     legend(legendName);
                    grid on
                    if(isAddTitle)
                        titleText = sprintf('Incremental Dynamic Analysis for Average (like roof) Drift Ratio, %s', meanAnalysisFolder);
                    end
                    title(titleText);
                    yLabel = sprintf('Sa_{g.m.}(T=1sec) [g]');
                    hy = ylabel(yLabel);
                    xLabel = sprintf('Roof Drift');
                    hx = xlabel(xLabel);
                    box on
                    FigureFormatScript

                    % Save the current figure
                    plotName = sprintf('IDA_AverageDrift.fig');
                    hgsave(plotName)
                    exportName = sprintf('IDA_AverageDrift.emf');
                    print('-dmeta', exportName);
                        ...
                        stripeIndex;
                            ...
                            for eqIndex = 1:length(maxStoryDriftRatioAllStripes{stripeIndex}{storyNum}.allEQs)
                                ...
                                if (isCurrentEQFullyConv == 1)
                                    ...
                                    % Plot the point
                                        plotVectorX = residualStoryDriftRatioAllStripes{stripeIndex}{storyNum}.allEQs(eqIndex);     % These are the individual EQ responces for the AllVar=Mean run.
                                        plotVectorY = saTOneForStripeLIST(stripeIndex);  % Just Sa level for stripe
                                        if (plotSingleEQPoints == 1)
                                            plot(plotVectorX, plotVectorY, markerType);
                                        end
                    ...
                    % Now plot the mean IDA, the mean +/- one sigmaRTR IDA, and the mean +/- one sigmaTOTAL IDA
                        % Plot the mean IDA line (the mean responce of the AllVar=Mean run)
                            ...
                            for stripeIndex = 1:length(saTOneForStripeLIST)
                                    ...
                                    meanResponceForPlot(1, stripeIndex + 1) = exp(residualStoryDriftRatioAllStripes{stripeIndex}{storyNum}.MeanLnConvEQs); % This leaves 
                    ...
                    % Now plot the IDA lines for +/- 1.0 sigmaRTR
                        ...
                        % Compute +/- one sigma RTR IDA lines
                            ...
                            for stripeIndex = 1:length(saTOneForStripeLIST)
                                ...
                                % Mean minum sigma
                                    meanMinusSigmaRTRResponceForPlot(1, stripeIndex + 1) = exp(residualStoryDriftRatioAllStripes{stripeIndex}{storyNum}.MeanLnConvEQs - ...
                                        residualStoryDriftRatioAllStripes{stripeIndex}{storyNum}.StDevLnConvEQs);
                                    
                                % Mean minum sigma
                                    meanPlusSigmaRTRResponceForPlot(1, stripeIndex + 1) = exp(residualStoryDriftRatioAllStripes{stripeIndex}{storyNum}.MeanLnConvEQs + ...
                                        residualStoryDriftRatioAllStripes{stripeIndex}{storyNum}.StDevLnConvEQs);          
...
%                     legend(legendName);
                    grid on
                    if(isAddTitle)
                        titleText = sprintf('Incremental Dynamic Analysis for Residual Story Drift Ratio of Story %d, %s', storyNum, meanAnalysisFolder);
                    end
                    title(titleText);
                    yLabel = sprintf('Sa_{g.m.}(T=1sec) [g]');
                    hy = ylabel(yLabel);
                    xLabel = sprintf('Residual Story Drift: Story %d', storyNum);
                    hx = xlabel(xLabel);
                    box on
                    FigureFormatScript

                    % Save the current figure
                    plotName = sprintf('IDA_ResidualDrift_Story_%d.fig', storyNum);
                    hgsave(plotName)
                    exportName = sprintf('IDA_ResidualDrift_Story_%d.emf', storyNum);
                    print('-dmeta', exportName);
...
%             end
        ...
        %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%% END REsidual STORY DRIFTS %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%

        
        %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%% START RESIDUAL ratio of STORY DRIFTS (resdidual drift / PTD) %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
        % Make the diagrams for all the story drifts...
            for storyNum = 1:maxStoryNum
                figure(figureNum)
                hold on
                        ...
                        stripeIndex;
                            ...
                            for eqIndex = 1:length(maxStoryDriftRatioAllStripes{stripeIndex}{storyNum}.allEQs)
                                ...
                                if (isCurrentEQFullyConv == 1)
                                    ...
                                    % Plot the point
                                        plotVectorX = residualStoryDriftRatioRatioAllStripes{stripeIndex}{storyNum}.allEQs(eqIndex);     % These are the individual EQ responces for the AllVar=Mean run.
                                        plotVectorY = saTOneForStripeLIST(stripeIndex);  % Just Sa level for stripe
                                        if (plotSingleEQPoints == 1)
                                            plot(plotVectorX, plotVectorY, markerType);
                                        end
                    ...
                    % Now plot the mean IDA, the mean +/- one sigmaRTR IDA, and the mean +/- one sigmaTOTAL IDA
                        % Plot the mean IDA line (the mean responce of the AllVar=Mean run)
                            ...
                            for stripeIndex = 1:length(saTOneForStripeLIST)
                                    ...
                                    meanResponceForPlot(1, stripeIndex + 1) = exp(residualStoryDriftRatioRatioAllStripes{stripeIndex}{storyNum}.MeanLnConvEQs); % This leaves 
                    ...
                    % Now plot the IDA lines for +/- 1.0 sigmaRTR
                        ...
                        % Compute +/- one sigma RTR IDA lines
                            ...
                            for stripeIndex = 1:length(saTOneForStripeLIST)
                                ...
                                % Mean minum sigma
                                    meanMinusSigmaRTRResponceForPlot(1, stripeIndex + 1) = exp(residualStoryDriftRatioRatioAllStripes{stripeIndex}{storyNum}.MeanLnConvEQs - ...
                                        residualStoryDriftRatioRatioAllStripes{stripeIndex}{storyNum}.StDevLnConvEQs);
                                    
                                % Mean minum sigma
                                    meanPlusSigmaRTRResponceForPlot(1, stripeIndex + 1) = exp(residualStoryDriftRatioRatioAllStripes{stripeIndex}{storyNum}.MeanLnConvEQs + ...
                                        residualStoryDriftRatioRatioAllStripes{stripeIndex}{storyNum}.StDevLnConvEQs);          
...
%                     legend(legendName);
                    grid on
                    if(isAddTitle)
                        titleText = sprintf('IDA for Ratio of Residual to Peak Transient Story Drift, Story %d, %s', storyNum, meanAnalysisFolder);
                    end
                    title(titleText);
                    yLabel = sprintf('Sa_{g.m.}(T=1sec) [g]');
                    hy = ylabel(yLabel);
                    xLabel = sprintf('Ratio of Residual to Peak Story Drift: Story %d', storyNum);
                    hx = xlabel(xLabel);
                    box on
                    FigureFormatScript

                    % Save the current figure
                    plotName = sprintf('IDA_RatioOfResidualToPeakTransDrift_Story_%d.fig', storyNum);
                    hgsave(plotName)
                    exportName = sprintf('IDA_RatioOfResidualToPeakTransDrift_Story_%d.emf', storyNum);
                    print('-dmeta', exportName);
...
%             end
        ...
        %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%% END REsidual ratio of DRIFTS (resdidual drift / PTD) %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
                    ...
                    % Now plot the mean IDA, the mean +/- one sigmaRTR IDA, and the mean +/- one sigmaTOTAL IDA
                        % Plot the mean IDA line (the mean responce of the AllVar=Mean run)
                            ...
                            for stripeIndex = 1:length(saTOneForStripeLIST)
                                    ...
                                    % Plot conv EQ's - PLOT THE FILTERED RESPONSE!
                                    meanResponceForPlot(1, stripeIndex + 1) = exp(maxFloorAccelAllStripes{stripeIndex}{floorNum}.MeanLnConvEQsUnfiltered) / g; % Changed to unfiltered on 10-6-04! 
                    ...
                    % Now plot the IDA lines for +/- 1.0 sigmaRTR
                        ...
                        % Compute +/- one sigma RTR IDA lines
                            ...
                            for stripeIndex = 1:length(saTOneForStripeLIST)
                                ...
                                % Mean minum sigma
                                % UNFILTERED
                                    meanMinusSigmaRTRResponceForPlot(1, stripeIndex + 1) = exp(maxFloorAccelAllStripes{stripeIndex}{floorNum}.MeanLnConvEQsUnfiltered - ...
                                        maxFloorAccelAllStripes{stripeIndex}{floorNum}.StDevLnConvEQsUnfiltered) / g;
                                    
                                % Mean minum sigma
                                    meanPlusSigmaRTRResponceForPlot(1, stripeIndex + 1) = exp(maxFloorAccelAllStripes{stripeIndex}{floorNum}.MeanLnConvEQsUnfiltered + ...
                                        maxFloorAccelAllStripes{stripeIndex}{floorNum}.StDevLnConvEQsUnfiltered) / g;
                            end                                                                                                     
...
%                     legend(legendName);
                    ...
                    yLabel = sprintf('Sa_{g.m.}(T=1sec) [g]');
                    hy = ylabel(yLabel);
                    xLabel = sprintf('Peak Floor Acceleration: Floor %d [g]', floorNum);
                    hx = xlabel(xLabel);
                    box on
                    FigureFormatScript

                    % Save the current figure
                    plotName = sprintf('IDA_PeakFloorAccel_Floor_%d.fig', floorNum);
                    hgsave(plotName)
                    exportName = sprintf('IDA_PeakFloorAccel_Floor_%d.emf', floorNum);
                    print('-dmeta', exportName);
                ...
                if (plotMeanLine == 1)
                    ...
                    % Now plot the mean IDA, the mean +/- one sigmaRTR IDA, and the mean +/- one sigmaTOTAL IDA
                        % Plot the mean IDA line (the mean responce of the AllVar=Mean run)
                            ...
                            % Loop for all stripe and populate the vector
                            for stripeIndex = 1:length(saTOneForStripeLIST)
                                meanResponceForPlot(1, stripeIndex + 1) = exp(maxColPHRInFullFrameAllStripes{stripeIndex}.maxHingeRot.MeanLnConvEQs); 
                            end                                                                                               
                    ...
                    % Now plot the IDA lines for +/- 1.0 sigmaRTR
                        ...
                        % Compute +/- one sigma RTR IDA lines
                            ...
                            for stripeIndex = 1:length(saTOneForStripeLIST)
                                ...
                                % Mean minum sigma
                                % UNFILTERED
                                    meanMinusSigmaRTRResponceForPlot(1, stripeIndex + 1) = exp(maxColPHRInFullFrameAllStripes{stripeIndex}.maxHingeRot.MeanLnConvEQs - ...
                                        maxColPHRInFullFrameAllStripes{stripeIndex}.maxHingeRot.StDevLnConvEQs);
                                    
                                % Mean minum sigma
                                    meanPlusSigmaRTRResponceForPlot(1, stripeIndex + 1) = exp(maxColPHRInFullFrameAllStripes{stripeIndex}.maxHingeRot.MeanLnConvEQs + ...
                                        maxColPHRInFullFrameAllStripes{stripeIndex}.maxHingeRot.StDevLnConvEQs);
                            end                                                                                                     
...
%                     legend(legendName);
                    ...
                    yLabel = sprintf('Sa_{g.m.}(T=1sec) [g]');
                    hy = ylabel(yLabel);
                    xLabel = sprintf('Peak Plastic Rotation: All Columns [rad]');
                    hx = xlabel(xLabel);
                    box on
                    FigureFormatScript

                    % Save the current figure
                    plotName = sprintf('IDA_PeakColumnPlasticRotationInFullFrame.fig');
                    hgsave(plotName)
                    exportName = sprintf('IDA_PeakColumnPlasticRotationInFullFrame.emf');
                    print('-dmeta', exportName);
                ...
                if (plotMeanLine == 1)
                    ...
                    % Now plot the mean IDA, the mean +/- one sigmaRTR IDA, and the mean +/- one sigmaTOTAL IDA
                        % Plot the mean IDA line (the mean responce of the AllVar=Mean run)
                            ...
                            % Loop for all stripe and populate the vector
                            for stripeIndex = 1:length(saTOneForStripeLIST)
                                meanResponceForPlot(1, stripeIndex + 1) = exp(maxBmPHRInFullFrameAllStripes{stripeIndex}.maxHingeRot.MeanLnConvEQs); 
                            end                                                                                               
                    ...
                    % Now plot the IDA lines for +/- 1.0 sigmaRTR
                        ...
                        % Compute +/- one sigma RTR IDA lines
                            ...
                            for stripeIndex = 1:length(saTOneForStripeLIST)
                                ...
                                % Mean minum sigma
                                % UNFILTERED
                                    meanMinusSigmaRTRResponceForPlot(1, stripeIndex + 1) = exp(maxBmPHRInFullFrameAllStripes{stripeIndex}.maxHingeRot.MeanLnConvEQs - ...
                                        maxBmPHRInFullFrameAllStripes{stripeIndex}.maxHingeRot.StDevLnConvEQs);
                                    
                                % Mean minum sigma
                                    meanPlusSigmaRTRResponceForPlot(1, stripeIndex + 1) = exp(maxBmPHRInFullFrameAllStripes{stripeIndex}.maxHingeRot.MeanLnConvEQs + ...
                                        maxBmPHRInFullFrameAllStripes{stripeIndex}.maxHingeRot.StDevLnConvEQs);
                            end                                                                                                     
...
%                     legend(legendName);
                    ...
                    yLabel = sprintf('Sa_{g.m.}(T=1sec) [g]');
                    hy = ylabel(yLabel);
                    xLabel = sprintf('Peak Plastic Rotation: All Beams [rad]');
                    hx = xlabel(xLabel);
                    box on
                    FigureFormatScript

                    % Save the current figure
                    plotName = sprintf('IDA_PeakBeamPlasticRotationInFullFrame.fig');
                    hgsave(plotName)
                    exportName = sprintf('IDA_PeakBeamPlasticRotationInFullFrame.emf');
                    print('-dmeta', exportName);
